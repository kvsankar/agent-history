...
- Do not collapse the agents into a single generic "AI Sessions" box;
  ...
  reflect.
- Do not depict workspace projects as a database join engine. Projects are a
...
- **Project**: user-defined name grouping multiple workspaces across
  homes/sources

This shared model is what `ws list`, `session list`, `session export`, and
`session stats` operate on. It
...
- **Projects Config**
  ...
  - small JSON/CFG file managed by `project` command
...
Agent Sessions across Homes (Local / WSL / Windows / SSH)
   ...
   Claude Adapter   Codex Adapter   Gemini Adapter
        ...
        +--------+--------+--------+--------+
        v        v        v        v        v
     ws list  session  session  session project
                list    export   stats
        |        |        |        |        |
        v        v        v        v        v
