...
def work_period_duration(
    timestamps: List[Tuple[str, str]], gap_threshold: float = GAP_THRESHOLD
) -> Tuple[float, int]:
    """
...
def analyze_workspace(ws_dir: Path) -> Dict:
    ...
    for session_id, files in sessions.items():
        ...
        all_timestamps.sort()
        calendar_time = (
            parse_timestamp(all_timestamps[-1]) - parse_timestamp(all_timestamps[0])
        ).total_seconds()
        ...
        if len(agent_files) >= 2:
            agent_times = [
                (parse_timestamp(a["first_ts"]), parse_timestamp(a["last_ts"])) for a in agent_files
            ]
...
def main():
    ...
    print(
        f"{'Session':<10} {'Msgs':>6} {'Agents':>6} {'Calendar':>10} {'Simple':>10} {'WorkPeriod':>10} {'Periods':>8} {'GapTime':>10}"
    )
    print("-" * 100)
    ...
    if total_simple > 0:
        ...
        print("  (Lower = more time spent in gaps between work periods)")
    ...
    for r in by_gap:
        if r["gap_time"] > 0:
            ...
            print(
                f"  {r['session_id']}: {format_duration(r['gap_time'])} gap time "
