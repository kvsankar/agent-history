...
Codex CLI, and Gemini CLI, organized by workspace. Supports all agents with auto-detection.
...
class WrappedHelpFormatter(argparse.RawDescriptionHelpFormatter):
    """Custom formatter that wraps usage lines at reasonable width."""

    def __init__(self, prog, indent_increment=2, max_help_position=30, width=80):
        super().__init__(prog, indent_increment, max_help_position, width)

    def _format_usage(self, usage, actions, groups, prefix):
        """Format usage with one option per line for readability."""
        if prefix is None:
            prefix = "usage: "
        ...
        # Build usage string with wrapped options
        lines = [f"{prefix}{prog}"]

        # Add optionals (abbreviated)
        if optionals:
            lines[0] += " [options]"
...
class ChoiceHintArgumentParser(argparse.ArgumentParser):
    """ArgumentParser that shows choices in error messages."""

    def error(self, message):
        """Show available choices when a choice argument is missing its value."""
        # Check if this is a "expected one argument" error for a choice argument
        if "expected one argument" in message:
            # Find the argument name from the message
            # Format: "argument --format: expected one argument"
            import re
            ...
            if match:
                ...
                for action in self._actions:
                    if arg_name in action.option_strings and action.choices:
                        choices_str = ", ".join(str(c) for c in action.choices)
                        message = f"argument {arg_name}: requires a value\n\nAvailable choices: {choices_str}"
                        break
...
def _default_jobs() -> int:
    """Return default worker count for parallel operations."""
...
DB_VERSION_PROJECT_COLUMNS = 6  # Version that adds git_remote_url, project, project_short
DB_VERSION_AGGREGATED_COLUMNS = 7  # Version that adds home + aggregated metrics tables
...
class SessionAggregates(TypedDict):
    """Aggregated counts/totals for a session."""
...
class ExportConfig:
    ...
    def from_args(cls, args, **overrides) -> "ExportConfig":
        ...
        return cls(
            ...
            export_json=get_field("export_json", False),
        )
...
class StatsCommandArgs:
    ...
    since: Optional[str] = None
    until: Optional[str] = None
    by: Optional[str] = None  # Unified dimension grouping (comma-separated)
    time: bool = False
    ...
    # Output formatting
    format: Optional[str] = None  # table, tsv, json
    human: bool = False  # Human-readable numbers and time
...
class SummaryStatsData:
    ...
    agent: str = "auto"
    output_format: Optional[str] = None  # table, tsv, json
...
# Stats dimension configuration
# Maps user-facing dimension names to SQL expressions
STATS_DIMENSIONS = {
    "home": {"sql": "s.source", "label": "Home", "width": 15},
    "agent": {"sql": "s.agent", "label": "Agent", "width": 8},
    "workspace": {"sql": "s.workspace", "label": "Workspace", "width": 30},
    "folder": {"sql": "s.workspace", "label": "Folder", "width": 20, "transform": "short_name"},
    "project": {"sql": "s.project", "label": "Project", "width": 30},
    "day": {"sql": "DATE(s.start_time)", "label": "Day", "width": 12},
    "model": {"sql": "m.model", "label": "Model", "width": 30, "requires_messages": True},
    "tool": {"sql": "t.tool_name", "label": "Tool", "width": 20, "requires_tools": True},
}
...
def matches_any_pattern(workspace_name: str, patterns: list[str]) -> bool:
    ...
    Patterns prefixed with '=' require an exact match.
    ...
    if any(p in _MATCH_ALL_PATTERNS for p in patterns_lower):
        return True

    # Exact match support (pattern startswith '=')
    for pattern, pattern_lower in zip(patterns, patterns_lower):
        ...
        if pattern.startswith("="):
            ...
            if workspace_lower == target_lower:
                return True
            try:
                normalized = normalize_workspace_name(workspace_name, verify_local=False).lower()
                if normalized == target_lower:
                    return True
            except Exception:
                # Best-effort normalization; fall back to substring
                pass

    # Check for substring matches
    return any(
        pattern and pat_lower in workspace_lower
        for pattern, pat_lower in zip(patterns, patterns_lower)
    )
...
def _matches_workspace_pattern(
    ...
    # Exact match prefix ('=') matches encoded or readable path exactly
    if pattern.startswith("="):
        target = pattern[1:]
        target_lower = target.lower()
        readable = get_readable(workspace) if get_readable else None
        readable_lower = readable.lower() if isinstance(readable, str) else None
        if target_lower in (workspace_lower, readable_lower):
            return True
        try:
            normalized = normalize_workspace_name(workspace, verify_local=False).lower()
            if normalized == target_lower:
                return True
        except Exception:
            pass
    ...
    looks_encoded = (
        ...
        or pattern_lower.startswith("home-")
        or pattern_lower.startswith("mnt-")
        or bool(re.match(r"^[A-Za-z]--", pattern_lower))
    )
    if looks_encoded and pattern_lower in workspace_lower:
        return True

    # If we don't have a readable representation, fall back to matching the raw identifier.
    if not readable and pattern_lower in workspace_lower:
        return True
    ...
    normalized_pattern = pattern_lower[1:] if pattern_lower.startswith("-") else pattern_lower
...
def _format_size_kb(session: dict) -> str:
    """Format size in KB with one decimal place."""
    size_kb = session.get("size_kb")
    if size_kb is None:
        return "-"
    try:
        return f"{float(size_kb):.1f} KB"
    except (TypeError, ValueError):
        return "-"


# ============================================================================
# Table Printer - Unified output formatting
# ============================================================================


# Global table width setting (set from CLI --width/-w)
_table_width: Optional[int] = None


def set_table_width(width: Optional[int]) -> None:
    """Set global table width for all TablePrinter instances."""
    global _table_width  # noqa: PLW0603
    _table_width = width


class TablePrinter:
    """Unified table printer supporting multiple output formats.

    Handles console (aligned columns), TSV, and JSON output formats.
    For console output, automatically calculates column widths and
    truncates long values with ellipsis to fit within screen width.
    """

    # Columns that contain paths - truncate from left to preserve unique part
    PATH_COLUMNS: ClassVar[set[str]] = {"WORKSPACE", "FILE", "PATH"}

    # Columns with fixed/small widths - don't trim these
    FIXED_COLUMNS: ClassVar[set[str]] = {
        "AGENT",
        "DATE",
        "MESSAGES",
        "SESSIONS",
    ...
    # Numeric columns - right-justify these
    NUMERIC_COLUMNS: ClassVar[set[str]] = {"MESSAGES", "SESSIONS", "WORKSPACES"}

    DEFAULT_WIDTH = 120  # Default screen width
    ...
    def __init__(
        ...
        Args:
            ...
            max_width: Maximum total table width (None=use global or default, 0=no limit)
            min_col_width: Minimum width for any column
            output_format: 'tsv', 'json', or None (console/table)
        """
        self.columns = columns
        # Priority: explicit param > global setting > default
        if max_width is not None:
            self.max_width = max_width
        elif _table_width is not None:
            self.max_width = _table_width
        else:
    ...
    def _truncate(self, text: str, width: int, from_left: bool = False) -> str:
        """Truncate text to width, adding ellipsis if needed.

        Args:
            text: Text to truncate
            width: Maximum width
            from_left: If True, truncate from left (for paths where end is important)
        """
        if len(text) <= width:
            return text
        if width <= 3:
            return text[:width] if not from_left else text[-width:]
        if from_left:
            return "..." + text[-(width - 3) :]
        return text[: width - 3] + "..."

    def _calculate_col_widths(self) -> list[int]:
        """Calculate optimal column widths for console output.

        Algorithm:
        1. Find max width per column by scanning all rows
        2. If total fits in screen width, use max widths
        3. If not, trim variable columns (paths) to fit, preserve fixed columns
        """
        # Step 1: Find max width per column (header or data)
        widths = [len(col) for col in self.columns]
        for row in self.rows:
            for i, col in enumerate(self.columns):
                widths[i] = max(widths[i], len(row.get(col, "")))

        # Step 2: Check if we fit
        spacing = (len(self.columns) - 1) * 2  # 2 spaces between cols
        total = sum(widths) + spacing

        # max_width=0 means no trimming
        if self.max_width == 0 or total <= self.max_width:
            return widths

        # Step 3: Trim variable columns (not fixed) to fit
        excess = total - self.max_width

        # Find variable columns (not in FIXED_COLUMNS), sorted by width desc
        variable_cols = [
            (i, widths[i])
            for i, col in enumerate(self.columns)
            if col not in self.FIXED_COLUMNS and widths[i] > self.min_col_width
        ]
        variable_cols.sort(key=lambda x: -x[1])  # widest first

        # Distribute the trim across variable columns
        for idx, width in variable_cols:
            if excess <= 0:
                break
            can_trim = width - self.min_col_width
            trim_by = min(excess, can_trim)
            widths[idx] -= trim_by
            excess -= trim_by
    ...
    def print(self) -> None:
        """Print the table in the configured format.

        If no format is specified and stdout is not a TTY (piped),
        automatically uses TSV for machine-readable output.
        """
        if self.output_format == "json":
            self._print_json()
        elif self.output_format == "tsv":
            self._print_tsv()
        elif self.output_format is None and not sys.stdout.isatty():
            # Auto-detect: use TSV when piped
            self._print_tsv()
        else:
            self._print_table()
    ...
    def _print_json(self) -> None:
        """Print as JSON array."""
        # Convert string values back to appropriate types where possible
        output = []
        for row in self.rows:
            typed_row = {}
            for col in self.columns:
                ...
                if val.isdigit():
                    typed_row[col.lower()] = int(val)
                else:
                    typed_row[col.lower()] = val
    ...
    def _print_table(self) -> None:
        """Print as aligned console table."""
        if not self.rows:
            return

        widths = self._calculate_col_widths()

        # Print header (right-justify numeric column headers)
        header_parts = []
        for i, col in enumerate(self.columns):
            if col in self.NUMERIC_COLUMNS:
                header_parts.append(col.rjust(widths[i]))
            else:
                header_parts.append(col.ljust(widths[i]))
        ...
        for row in self.rows:
            ...
            for i, col in enumerate(self.columns):
                val = row.get(col, "")
                # Truncate paths from left to preserve unique suffix
                from_left = col in self.PATH_COLUMNS
                truncated = self._truncate(val, widths[i], from_left=from_left)
                # Right-justify numeric columns
                if col in self.NUMERIC_COLUMNS:
                    row_parts.append(truncated.rjust(widths[i]))
                else:
                    row_parts.append(truncated.ljust(widths[i]))
...
def _aggregate_workspaces(sessions: list) -> dict:
    """Aggregate sessions by workspace to compute counts and last modified.

    Returns:
        Dict mapping workspace_readable to {sessions: count, last_modified: datetime}
    """
    workspaces = {}
    for s in sessions:
        ws = s.get("workspace_readable") or s.get("workspace")
        if ws not in workspaces:
            workspaces[ws] = {"sessions": 0, "last_modified": None}
        workspaces[ws]["sessions"] += 1
        modified = s.get("modified")
        if modified:
            if (
                workspaces[ws]["last_modified"] is None
                or modified > workspaces[ws]["last_modified"]
            ):
                workspaces[ws]["last_modified"] = modified
    return workspaces


def _check_workspace_status(workspace_path: str, source_label: str) -> str:
    """Check if workspace path exists on filesystem.

    Args:
        workspace_path: Decoded workspace path
        source_label: Source label to determine base path

    Returns:
        'ok' if exists, 'missing' if not, 'unknown' if path cannot be determined
    """
    ...
    if source_label.startswith("remote:") or source_label == "web":
        return "ok"
    ...
    candidates: list[Path] = [check_path]

    # If encoded workspace, try decoded path as well to avoid false "missing"
    if clean_path.startswith("-") or _is_windows_encoded_path(clean_path):
        try:
            decoded = normalize_workspace_name(clean_path, verify_local=True)
        except Exception:
            decoded = normalize_workspace_name(clean_path, verify_local=False)
        if decoded:
            decoded_path = Path(agent_home) / decoded.lstrip("/") if agent_home else Path(decoded)
...
def _normalize_home_label(source_label: str) -> str:
    """Normalize source label to short home identifier."""
    if source_label in ("Local (WSL)", "Local"):
        return "local"
    if source_label.startswith("Windows"):
        return "windows"
    if source_label.startswith("Remote"):
        # Extract hostname from "Remote (hostname)"
        if "(" in source_label and ")" in source_label:
            host = source_label.split("(")[1].split(")")[0]
            return f"remote:{host}"
        return "remote"
    if source_label.startswith("WSL"):
        if "(" in source_label and ")" in source_label:
            distro = source_label.split("(")[1].split(")")[0]
            return f"wsl:{distro}"
        return "wsl"
    return source_label


def print_sessions_output(
    sessions: list,
    source_label: str,
    ...
    Args:
        ...
        workspaces_only: If True, print workspace summary (HOME, WORKSPACE, SESSIONS, STATUS, LAST MODIFIED)
        output_format: table, tsv, or json (default: table)
        max_width: Maximum table width (None=default, 0=no limit)
    """
    home_label = _normalize_home_label(source_label)

    if workspaces_only:
        columns = ["HOME", "WORKSPACE", "SESSIONS", "STATUS", "LAST_MODIFIED"]
        printer = TablePrinter(columns, output_format=output_format, max_width=max_width)

        workspaces = _aggregate_workspaces(sessions)
        for ws in sorted(workspaces.keys()):
            data = workspaces[ws]
            status = _check_workspace_status(ws, source_label)
            last_mod = (
                data["last_modified"].strftime("%Y-%m-%d %H:%M") if data["last_modified"] else ""
            )
            printer.add_row(
                {
                    "HOME": home_label,
        ...
        columns = ["AGENT", "HOME", "WORKSPACE", "FILE", "MESSAGES", "DATE"]
        printer = TablePrinter(columns, output_format=output_format, max_width=max_width)
        home_label = _source_label_to_home(source_label)
            ...
            printer.add_row(
                {
                    ...
                    "FILE": session.get("filename") or session.get("file"),
                    "MESSAGES": _format_message_count(session),
                    "DATE": session["modified"].strftime("%Y-%m-%d")
        ...
        columns = ["SESSION", "MESSAGES", "SIZE", "MODIFIED"]
        printer = TablePrinter(columns, output_format=output_format, max_width=max_width)

        for session in sessions:
            printer.add_row(
                {
                    "SESSION": session.get("filename") or session.get("file"),
                    "MESSAGES": _format_message_count(session),
                    "SIZE": _format_size_kb(session),
...
class ConversationGraph:
    ...
    messages: list
    uuid_to_msg: dict
    uuid_to_children: dict
    fork_points: list  # List of UUIDs that have multiple children
    branches: list  # List of branch descriptions
    is_linear: bool  # True if no forks


def analyze_conversation_graph(messages: list) -> ConversationGraph:
    """Analyze conversation structure to detect forks and branches.

    Args:
        messages: List of message dicts with uuid and parentUuid fields

    Returns:
        ConversationGraph with fork analysis
    """
    ...
    # Find fork points (parents with multiple children)
    ...
    for fork_uuid in fork_points:
        ...
        for child_uuid in children:
            ...
            if child_msg:
                branches_list = branch_info["branches"]
...
def generate_graph_summary(graph: ConversationGraph) -> list:
    """Generate markdown summary of conversation graph structure.

    Args:
        graph: ConversationGraph from analyze_conversation_graph

    Returns:
        List of markdown lines describing the graph structure
    """
    if graph.is_linear:
        return []
    ...
    for i, branch_info in enumerate(graph.branches, 1):
        ...
        fork_ts = branch_info["fork_timestamp"]
        fork_type = branch_info["fork_type"]
        ...
        for j, branch in enumerate(branch_info["branches"], 1):
            ...
            branch_ts = branch["timestamp"]
            branch_type = branch["type"]
            # Create link if uuid exists
            lines.append(
...
def generate_mermaid_graph(graph: ConversationGraph, max_nodes: int = 50) -> list:
    """Generate Mermaid diagram of conversation graph.

    Args:
        graph: ConversationGraph from analyze_conversation_graph
        max_nodes: Maximum number of nodes to include (truncates large graphs)

    Returns:
        List of markdown lines with Mermaid diagram
    """
    ...
    # Only include nodes near fork points to keep diagram readable
    relevant_uuids = set()
    ...
    for uuid in relevant_uuids:
        msg = graph.uuid_to_msg.get(uuid)
        if msg and msg.get("parentUuid"):
            parent = msg["parentUuid"]
            if parent in [u[:6] for u in relevant_uuids] or any(
                u.startswith(parent[:6]) for u in relevant_uuids
            ):
                # Find matching parent
                parent_short = None
                for u in relevant_uuids:
    ...
    # Highlight fork points
    for fork_uuid in graph.fork_points:
        if fork_uuid in relevant_uuids or any(u.startswith(fork_uuid[:6]) for u in relevant_uuids):
...
def parse_jsonl_to_markdown(
    ...
    Args:
        ...
        show_graph: If True, include conversation graph analysis for forked conversations
    """
    if messages is None:
    ...
    # Add conversation graph analysis if there are forks (skip in minimal mode)
    if show_graph and not minimal and len(messages) > 1:
...
def _generate_message_metadata(msg: dict, uuid_to_index: dict) -> list[str]:
    ...
    if msg.get("isMeta") is not None:
        md_lines.append(f"- **Is Meta:** `{msg['isMeta']}`")
...
def _extract_unified_content(msg: dict, agent_type: str) -> str:
    """Extract content string from agent-specific message format.

    Args:
        msg: Message dict from agent-specific format
...
def _claude_message_to_unified(msg: dict) -> dict:
    ...
    unified = {
        ...
        "content": _extract_unified_content(msg, AGENT_CLAUDE),
    }
...
def _codex_message_to_unified(msg: dict) -> dict:
...
def parse_to_unified_ndjson(
    session_file: Path, agent_type: str, messages: Optional[list] = None
) -> list:
    """Parse session file to unified NDJSON format.
    ...
    Returns:
        List of unified schema dicts ready for NDJSON output
    """
    if messages is None:
        if agent_type == AGENT_GEMINI:
            ...
            messages = read_jsonl_messages(session_file, quiet=True)

    unified_records = []
    for msg in messages:
        if agent_type == AGENT_GEMINI:
            unified_records.append(_gemini_message_to_unified(msg))
        elif agent_type == AGENT_CODEX:
            unified_records.append(_codex_message_to_unified(msg))
        else:
            unified_records.append(_claude_message_to_unified(msg))

    return unified_records


def _infer_workspace_for_unified(session_file: Path, agent_type: str) -> Optional[str]:
    """Best-effort workspace inference for unified export header."""
    try:
        if agent_type == AGENT_CLAUDE:
            return normalize_workspace_name(session_file.parent.name, verify_local=True)
        if agent_type == AGENT_CODEX:
            return codex_get_workspace_from_session(session_file)
        if agent_type == AGENT_GEMINI:
            return gemini_get_workspace_from_session(session_file)
    except Exception:
        return None
    return None


def _build_unified_header(agent_type: str, workspace: Optional[str] = None) -> dict:
    """Build header record for unified NDJSON export."""
    agent_map = {
    ...
    return {
        ...
        "export_timestamp": datetime.now(timezone.utc).replace(microsecond=0).isoformat() + "Z",
        "exporter_version": __version__,
        "agent_types": [agent_map.get(agent_type, agent_type)],
        "homes": ["local"],
...
def write_unified_ndjson(
    records: list, output_file: Path, quiet: bool = False, header: Optional[dict] = None
) -> None:
    """Write unified records to NDJSON file.

    Args:
        records: List of unified schema dicts
        output_file: Output file path (should have .ndjson extension)
        header: Optional header record to write first
...
def get_active_backends(agent: str, projects_dir: Optional[Path] = None) -> list:
    """Return list of active backends based on agent flag.

    Args:
        agent: Agent selection - 'auto', 'claude', 'codex', or 'gemini'
        projects_dir: Optional override for Claude projects directory (for testing)

    Returns:
        List of backend identifiers (AGENT_CLAUDE, AGENT_CODEX, AGENT_GEMINI) that exist
    """
    claude_projects = projects_dir if projects_dir else _get_claude_projects_path()
    ...
    **kwargs,
    ...
    Args:
        ...
        **kwargs: Additional arguments passed to backend functions
            projects_dir: Optional override for Claude projects directory
    ...
    backends = get_active_backends(agent, projects_dir=projects_dir)
...
def get_source_tag(remote_spec: Optional[str] = None) -> str:
    ...
    if is_windows_remote(remote_spec):
        ...
        elif remote_spec.startswith("windows:"):
            username = remote_spec[8:]  # Remove 'windows:' prefix
            return f"windows_{username.lower()}_"
        else:
            return "windows_"
...
def get_agent_windows_dir(username: Optional[str], agent: str):
    ...
    If AGENT_HISTORY_HOME_WINDOWS is set, uses that path to construct the agent
    directory instead of probing real Windows filesystem (for testing).
    """
    # Check for test override - use injected Windows home path
    windows_home_override = os.environ.get("AGENT_HISTORY_HOME_WINDOWS")
    if windows_home_override:
        home = Path(windows_home_override)
        if agent == AGENT_CLAUDE:
...
def gemini_fetch_remote_hash_index(remote_host: Optional[str], local_cache_dir: Path) -> dict:
    ...
    try:
        ...
        check_cmd = (
            "if [ -f ~/.agent-history/gemini_index.json ]; then "
            "cat ~/.agent-history/gemini_index.json; "
            "elif [ -f ~/.agent-history/gemini_hash_index.json ]; then "
...
# ============================================================================
# Web Sessions API (Claude.ai)
# ============================================================================

WEB_API_BASE_URL = "https://api.anthropic.com/v1"
WEB_API_VERSION = "2023-06-01"
WEB_DISPLAY_TITLE_MAX_LEN = 50


class WebSessionsError(Exception):
    """Error accessing web sessions API."""
...
def get_access_token_from_keychain() -> Optional[str]:
    ...
    Returns:
        Access token string or None if not found/not macOS.
    """
...
def get_access_token_from_credentials_file() -> Optional[str]:
    """Get access token from ~/.claude/.credentials.json.

    This is the file-based credential storage used by Claude Code on Linux/Windows.
...
def get_access_token() -> Optional[str]:
    ...
    2. ~/.claude/.credentials.json (file-based, all platforms)
...
def _make_web_api_request(endpoint: str, token: str, org_uuid: str, timeout: int = 30) -> dict:
    """Make a request to the Claude web API.
    ...
    Raises:
        WebSessionsError: On network or API errors
    """
    import urllib.error
    ...
    try:
        ...
        raise WebSessionsError(f"API error {e.code}: {e.reason}") from e
    except urllib.error.URLError as e:
        raise WebSessionsError(f"Network error: {e.reason}") from e
    except json.JSONDecodeError as e:
        raise WebSessionsError(f"Invalid JSON response: {e}") from e


def fetch_web_sessions(token: str, org_uuid: str) -> list:
    ...
    Returns:
        List of session dictionaries with 'id', 'title', 'created_at', etc.

    Raises:
        WebSessionsError: On API errors
    """
...
# ============================================================================
#
# Workspace URIs uniquely identify workspaces across different sources using
# familiar, existing formats. The general pattern is:
#
#   primary_identifier[@github.com/owner/repo]
#
# Where primary_identifier uses context-appropriate familiar formats:
#
# LOCAL PATHS (filesystem path):
#   /home/user/project
#   /home/user/project@github.com/owner/repo      # with GitHub correlation
#
# REMOTE (SSH format):
#   user@host:/path/to/project
#   user@host:/path@github.com/owner/repo
#
# WSL (simplified UNC-like):
#   wsl:Ubuntu:/home/user/project
#   wsl:Ubuntu:/home/user/project@github.com/owner/repo
#
# WINDOWS (native path format):
#   C:\Users\name\project
#   windows:username:C:\Users\name\project        # when accessed from WSL
#
# WEB SESSIONS (URL format):
#   claude.ai/session/session_011CUvz9KgREyL4RPDmtxkJW
#   claude.ai/session/session_xyz@github.com/owner/repo
#
# GEMINI (URN format for unmapped hashes):
#   urn:gemini:321784d9
#   /home/user/project@gemini:321784d9            # mapped to path
#
# The @suffix provides correlation to GitHub repos, enabling:
# - Matching web sessions to local workspaces
# - Identifying the same project across different machines
# - Linking sessions from different sources
#
# ============================================================================
...
def extract_github_repo_from_git_url(url: str) -> Optional[str]:
    ...
    - github-alias:owner/repo.git (SSH alias)
    ...
    Returns:
        Repository in 'owner/repo' format, or None if not GitHub
    """
    if not url:
        return None
    ...
    # SSH format: git@github.com:owner/repo.git or git@github-alias:owner/repo.git
    # Also: github-alias:owner/repo.git (without git@)
    if ":" in url and "/" in url.split(":")[-1]:
        # Check if it looks like a GitHub SSH URL (contains 'github' in host part)
...
def normalize_git_url(url: str) -> tuple[Optional[str], Optional[str]]:
    """Normalize git URL to canonical and short forms.

    Handles various git URL formats from any hosting provider:
    - SSH: git@host:owner/repo.git
    - HTTPS: https://host/owner/repo.git
    - SSH with explicit ssh://: ssh://git@host/owner/repo.git

    Args:
        url: Git remote URL

    Returns:
        Tuple of (canonical, short) where:
        - canonical: host/owner/repo (e.g., github.com/user/repo)
        - short: owner/repo (e.g., user/repo)
    ...
    # HTTPS/HTTP/SSH with protocol format: https://host/owner/repo.git
    # Match first to avoid SSH pattern matching protocol URLs
    url_match = re.match(r"^(?:https?|ssh)://(?:[^@]+@)?([^/]+)/(.+)$", url)
    if url_match:
        host = url_match.group(1)
        path = url_match.group(2).rstrip("/").removesuffix(".git")
        canonical = f"{host}/{path}"
        short = path
        return canonical, short

    # SSH format: git@host:owner/repo.git or host:owner/repo.git
    # Match pattern like: git@github.com:user/repo.git or github-alias:user/repo.git
    # The path must contain / to distinguish from port numbers (host:port)
    ssh_match = re.match(r"^(?:git@)?([^:]+):([^/]+/.+)$", url)
    if ssh_match:
        host = ssh_match.group(1)
        path = ssh_match.group(2).rstrip("/").removesuffix(".git")
        canonical = f"{host}/{path}"
        short = path
        return canonical, short
...
def get_git_remote_for_workspace(
    workspace_path: Path,
) -> tuple[Optional[str], Optional[str], Optional[str]]:
    """Extract git remote URL and derive project names for a workspace.

    Args:
        workspace_path: Path to the workspace directory

    Returns:
        Tuple of (git_remote_url, project_canonical, project_short):
        - git_remote_url: Raw git remote URL (e.g., git@github.com:user/repo.git)
        - project_canonical: Normalized canonical name (e.g., github.com/user/repo)
        - project_short: Short name (e.g., user/repo)
        Returns (None, None, None) if git remote cannot be extracted
    """
    ...
    try:
        ...
        canonical, short = normalize_git_url(git_remote_url)
        return git_remote_url, canonical, short
...
def get_web_session_github_repo(session: dict) -> Optional[str]:
    """Extract GitHub repo (owner/repo) from a web session.

    Args:
        session: Session dictionary from fetch_web_sessions
    ...
    # Check sources for git repo URL
    sources = ctx.get("sources", [])
    for src in sources:
        if src.get("type") == "git_repository":
            url = src.get("url", "")
            repo = extract_github_repo_from_git_url(url)
            if repo:
                return repo

    # Fallback: check outcomes for git_info.repo (already in owner/repo format)
    outcomes = ctx.get("outcomes", [])
    for out in outcomes:
        git_info = out.get("git_info", {})
...
def build_github_to_workspace_map(projects_dir: Path) -> dict:
    """Build a map of GitHub repos to local workspace paths.

    Scans local workspaces for git remotes and extracts GitHub repos.

    Args:
        projects_dir: Path to Claude projects directory

    Returns:
        Dict mapping 'owner/repo' to local workspace path
    """
    repo_map: dict = {}
    ...
    for workspace_dir in projects_dir.iterdir():
        if not workspace_dir.is_dir():
            continue
        # Skip cached remote/WSL workspaces
        if workspace_dir.name.startswith(("remote_", "wsl_")):
            continue

        # Convert workspace dir name to actual path
        workspace_path = normalize_workspace_name(workspace_dir.name)

        # Check if this path has a .git directory
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                repo = extract_github_repo_from_git_url(remote_url)
                if repo:
...
def get_web_session_workspace(session: dict, github_map: Optional[dict] = None) -> Optional[str]:
    """Extract workspace identifier from a web session.

    If a github_map is provided, attempts to resolve GitHub repo to local path.

    Args:
        session: Session dictionary from fetch_web_sessions
        github_map: Optional dict mapping 'owner/repo' to local workspace path

    Returns:
        Local workspace path if matched, otherwise GitHub repo, or None
    """
    github_repo = get_web_session_github_repo(session)

    if github_repo:
        # Try to resolve to local workspace path
        if github_map and github_repo in github_map:
            return github_map[github_repo]
        return github_repo

    # Fallback: cwd if available
    ctx = session.get("session_context", {})
    if isinstance(ctx, str):
        try:
            ctx = json.loads(ctx)
...
# --- Workspace URI Functions ---


@dataclass
class WorkspaceURI:
    """Parsed workspace URI components."""

    location: str  # local, remote, wsl, windows, web, gemini
    host: Optional[str] = None  # hostname for remote, distro for wsl, user for windows
    path: Optional[str] = None  # filesystem path
    session_id: Optional[str] = None  # web session ID
    github_repo: Optional[str] = None  # owner/repo correlation
    gemini_hash: Optional[str] = None  # Gemini workspace hash


def make_workspace_uri(
    location: str,
    ...
    """Generate a workspace URI string.

    Args:
        location: One of 'local', 'remote', 'wsl', 'windows', 'web', 'gemini'
        path: Filesystem path (for local, remote, wsl, windows)
        host: Hostname (remote), distro (wsl), or username (windows)
        session_id: Web session ID
        github_repo: GitHub owner/repo for correlation
        gemini_hash: Gemini workspace hash

    Returns:
        Workspace URI string in familiar format
    """
    uri = ""

    if location == "local":
        # /home/user/project[@github.com/owner/repo]
        uri = path or ""
        ...
        # windows:username:C:\Users\...  or just C:\...
        if host and path:
            uri = f"windows:{host}:{path}"
        elif path:
            uri = path
    elif location == "web":
        # claude.ai/session/session_id
        if session_id:
            uri = f"claude.ai/session/{session_id}"
        else:
            uri = "claude.ai"
    elif location == "gemini":
        # urn:gemini:hash or path@gemini:hash
        if path:
            uri = path
        elif gemini_hash:
            uri = f"urn:gemini:{gemini_hash}"

    # Add GitHub correlation suffix
    if github_repo:
        uri = f"{uri}@github.com/{github_repo}"

    # Add Gemini hash suffix (if path is primary)
    if gemini_hash and path:
        uri = f"{uri}@gemini:{gemini_hash}"

    return uri


def parse_workspace_uri(uri: str) -> WorkspaceURI:
    """Parse a workspace URI string into components.

    Args:
        uri: Workspace URI string

    Returns:
        WorkspaceURI with parsed components
    """
    github_repo = None
    gemini_hash = None

    # Extract @github.com/owner/repo suffix
    if "@github.com/" in uri:
        uri, github_part = uri.rsplit("@github.com/", 1)
        # Handle @gemini: suffix that may come after github repo
        if "@gemini:" in github_part:
            github_repo, gemini_hash = github_part.split("@gemini:", 1)
        else:
            github_repo = github_part.split("@")[0]  # In case there are other suffixes

    # Extract @gemini:hash suffix (if not already extracted from github_part)
    if gemini_hash is None and "@gemini:" in uri:
        uri, gemini_hash = uri.rsplit("@gemini:", 1)

    # Determine location from URI format
    if uri.startswith("claude.ai/session/"):
        session_id = uri.replace("claude.ai/session/", "")
        return WorkspaceURI(
            location="web",
            session_id=session_id,
            github_repo=github_repo,
            gemini_hash=gemini_hash,
        )

    if uri.startswith("urn:gemini:"):
        gemini_hash = uri.replace("urn:gemini:", "")
        return WorkspaceURI(location="gemini", gemini_hash=gemini_hash, github_repo=github_repo)

    if uri.startswith("wsl:"):
        # wsl:Ubuntu:/path or wsl:/path
        parts = uri[4:].split(":", 1)
        if len(parts) == 2:
            return WorkspaceURI(
                location="wsl",
    ...
    # Check for remote SSH format: user@host:/path or host:/path
    if ":" in uri and not uri.startswith("/"):
        # Could be remote SSH format
    ...
    # Default: local path
    return WorkspaceURI(
        location="local", path=uri, github_repo=github_repo, gemini_hash=gemini_hash
    )


def workspace_uri_from_source(
    source_label: str,
    workspace_readable: str,
    github_repo: Optional[str] = None,
    gemini_hash: Optional[str] = None,
) -> str:
    """Generate a workspace URI from source label and workspace path.

    Args:
        source_label: Source label like "Local", "Local (WSL)", "Windows (kvsan)",
                     "WSL (Ubuntu)", "Remote: hostname"
        workspace_readable: The readable workspace path
        github_repo: Optional GitHub repo for correlation
        gemini_hash: Optional Gemini hash

    Returns:
        Workspace URI string
    """
    # Check for Gemini hash in workspace_readable (e.g., "[hash:321784d9]")
    if workspace_readable.startswith("[hash:") and workspace_readable.endswith("]"):
        hash_val = workspace_readable[6:-1]  # Extract hash from [hash:xxx]
        return make_workspace_uri(location="gemini", gemini_hash=hash_val)

    # Parse source label to determine location type and host
    label_lower = source_label.lower()

    if label_lower.startswith("local"):
        return make_workspace_uri(
            location="local",
    ...
    if label_lower.startswith("windows"):
        # Extract username from "Windows (username)"
        host = None
        if "(" in source_label and ")" in source_label:
            host = source_label.split("(")[1].split(")")[0]
        return make_workspace_uri(
    ...
    if label_lower.startswith("wsl"):
        # Extract distro from "WSL (Ubuntu)" or similar
        host = None
        if "(" in source_label and ")" in source_label:
            host = source_label.split("(")[1].split(")")[0]
        return make_workspace_uri(
    ...
    if label_lower.startswith("remote"):
        # Extract hostname from "Remote: hostname" or "Remote (hostname)"
        host = None
        if ":" in source_label:
            host = source_label.split(":", 1)[1].strip()
    ...
    if label_lower.startswith("web"):
        return make_workspace_uri(
            location="web",
            session_id=workspace_readable,
            github_repo=github_repo,
        )
...
def fetch_web_session(token: str, org_uuid: str, session_id: str) -> dict:
    """Fetch a specific web session from the Claude API.
    ...
    Returns:
        Session data dictionary with 'loglines', etc.
...
def web_session_to_jsonl(session_data: dict) -> str:
    """Convert web session data to JSONL format.

    Args:
        session_data: Session data from fetch_web_session
...
def resolve_web_credentials(
    ...
    """Resolve web API credentials from arguments or auto-detect.

    Checks for credentials in order:
    1. Explicit token/org_uuid arguments
    2. macOS keychain (if on macOS)
    3. ~/.claude/.credentials.json (file-based, all platforms)
    4. ~/.claude.json for org UUID

    Args:
        token: Optional access token (auto-detected if None)
        org_uuid: Optional org UUID (uses ~/.claude.json if None)

    Returns:
        Tuple of (token, org_uuid)

    Raises:
        WebSessionsError: If credentials cannot be resolved
    """
    # Get token
    if token is None:
        ...
        if token is None:
            raise WebSessionsError(
                ...
                "Make sure you are logged into Claude Code (check ~/.claude/.credentials.json)."
            )
...
def _build_workspace_matcher(workspace: str) -> Callable[[dict], bool]:
    """Build a predicate to check whether a session belongs to the target workspace."""
    targets = {workspace}
    # Add both verified and unverified normalized forms (avoid hyphen loss for local paths)
    for verify_local in (True, False):
        try:
            normalized = normalize_workspace_name(workspace, verify_local=verify_local)
        except Exception:
            normalized = None
        if normalized:
            targets.add(normalized)
    base = get_workspace_name_from_path(workspace)
    if base:
        targets.add(base)
    for t in list(targets):
        norm_base = get_workspace_name_from_path(t)
        if norm_base:
            targets.add(norm_base)
    targets = {str(t) for t in targets if t}

    def matches(session: dict) -> bool:
        ws = str(session.get("workspace") or "")
        readable = str(session.get("workspace_readable") or "")

        # Direct match against known variants
        if ws in targets or readable in targets:
            return True

        # Match decoded/normalized forms
        decoded_ws = normalize_workspace_name(ws, verify_local=False) if ws else ""
        if decoded_ws in targets:
            return True

        base_ws = get_workspace_name_from_path(ws) if ws else ""
        if base_ws in targets:
            return True

        if readable:
            readable_base = get_workspace_name_from_path(readable)
            if readable_base in targets:
                return True
...
def _get_all_agent_sessions_for_source(
    source_key: str, workspace: str, since_date=None, until_date=None
) -> list:
    """Gather sessions for all agents for a given source/workspace."""
    sessions: list[dict] = []
    agents = [AGENT_CLAUDE, AGENT_CODEX, AGENT_GEMINI]
    matches_target = _build_workspace_matcher(workspace)

    # Build pattern variants to catch encoded/decoded/basename forms
    patterns: list[str] = []
    for candidate in [workspace, normalize_workspace_name(workspace, verify_local=False)]:
        if candidate and candidate not in patterns:
            patterns.append(candidate)
            base = get_workspace_name_from_path(candidate)
            if base and base not in patterns:
                patterns.append(base)

    seen_files: set[str] = set()

    def _seen(session: dict) -> bool:
        """Deduplicate by file path/filename."""
        key = str(session.get("file") or session.get("filename") or "")
        if not key:
            return False
        ...
        try:
            if source_key == "local":
                for pattern in patterns:
                    for s in collect_sessions_with_dedup(
                        [pattern],
                        since_date=since_date,
                        until_date=until_date,
                ...
                distro = source_key.split(":", 1)[1]
                sessions_dir = get_agent_wsl_dir(distro, agent_name)
                if not sessions_dir or not sessions_dir.exists():
                    continue
                if agent_name == AGENT_CLAUDE:
                    for pattern in patterns:
                        for s in get_workspace_sessions(
                            pattern,
                            quiet=True,
                            since_date=since_date,
                            until_date=until_date,
                    ...
                    for s in _scan_codex_gemini_sessions(
                        agent_name,
                        patterns,
                        since_date,
                        until_date,
                ...
                user = source_key.split(":", 1)[1] if ":" in source_key else None
                sessions_dir = get_agent_windows_dir(user, agent_name)
                if not sessions_dir or not sessions_dir.exists():
                    continue
                if agent_name == AGENT_CLAUDE:
                    for pattern in patterns:
                    ...
                    for s in _scan_codex_gemini_sessions(
                        ...
                        if _seen(s):
                            continue
                        sessions.append(s)
            elif source_key.startswith("remote:") or "@" in source_key:
                remote_spec = (
                    source_key.split(":", 1)[1] if source_key.startswith("remote:") else source_key
                )
                if agent_name == AGENT_CLAUDE:
                    for s in _collect_remote_claude_sessions_simple(
                        remote_spec, patterns, since_date, until_date
                    ):
                        if not matches_target(s):
                            continue
                        if _seen(s):
                            continue
                        sessions.append(s)
                elif agent_name == AGENT_CODEX:
                    for s in _collect_remote_codex_sessions_simple(
                        remote_spec, patterns, since_date, until_date
                    ):
                        if not matches_target(s):
                            continue
                        if _seen(s):
                            continue
                        sessions.append(s)
                elif agent_name == AGENT_GEMINI:
                    for s in _collect_remote_gemini_sessions_simple(
                        remote_spec, patterns, since_date, until_date
                    ):
                        if not matches_target(s):
                            continue
                ...
                # Web sessions are only Claude-format; reuse existing helper
                for s in get_sessions_for_source(source_key, workspace):
                    if not matches_target(s):
                        continue
                    if _seen(s):
                        continue
...
def cmd_project_list(args):
    """List all projects with their workspaces (flat format)."""
    aliases_data = load_aliases()
    aliases = aliases_data.get("projects", {})
    show_counts = getattr(args, "counts", False)
    output_format = getattr(args, "format", None)

    if not aliases:
        sys.stderr.write("No projects defined.\n")
        sys.stderr.write("\nAdd a project with:\n")
        sys.stderr.write("  agent-history project add <name> <workspace>\n")
        return
    ...
    # Define columns based on whether counts are shown
    if show_counts:
        columns = ["PROJECT", "SOURCE", "WORKSPACE", "SESSIONS"]
    else:
        columns = ["PROJECT", "SOURCE", "WORKSPACE"]

    # Create TablePrinter
    printer = TablePrinter(columns, output_format=output_format)

    # Collect all data and add to printer
    for alias_name, sources in sorted(aliases.items()):
        if not sources:
            # Empty project - show with empty source/workspace
            row = {
                "PROJECT": alias_name,
                "SOURCE": "",
                "WORKSPACE": "",
...
def cmd_project_show(args):
    ...
    if alias_name is None:
        # Derive from current workspace if possible
        pattern, exists = check_current_workspace_exists()
        if exists:
            guessed = get_alias_for_workspace(pattern, "local")
            if guessed:
                alias_name = guessed
    if alias_name is None:
        exit_with_error("Project name not specified and current workspace is not in a project.")
    if alias_name not in aliases:
        sys.stderr.write(f"Error: Project '{alias_name}' not found\n")
        sys.exit(1)

    sources = aliases[alias_name]

    # Collect workspace info and compute totals
    total_sessions = 0
    last_modified = None
    workspace_lines = []
    ...
    for source_key, workspaces in sorted(sources.items()):
        for workspace in workspaces:
            sessions = _get_all_agent_sessions_for_source(
                source_key, workspace, since_date=None, until_date=None
            )
            ws_readable = normalize_workspace_name(workspace)
            session_count = len(sessions)
            total_sessions += session_count
            workspace_lines.append((source_key, ws_readable, session_count))
            per_source.setdefault(source_key, {"sessions": 0, "messages": 0})
            per_source[source_key]["sessions"] += session_count

            for session in sessions:
                agent_name = session.get("agent") or "claude"
                per_agent[agent_name] = per_agent.get(agent_name, 0) + 1
                msg_cnt = session.get("message_count", 0)
                per_source[source_key]["messages"] += msg_cnt

            # Track latest modified time
            for session in sessions:
                mod_time = session.get("modified")
                if mod_time:
                    if last_modified is None or mod_time > last_modified:
                        last_modified = mod_time

    # Print project details in spec format
    print(f"Project: {alias_name}")
    print()
    print("Workspaces:")
    for source_key, workspace, count in workspace_lines:
        print(f"  {source_key:16}  {workspace}")
    print()
    ...
    if last_modified:
        ...
        try:
            if isinstance(last_modified, str):
                ...
                formatted = str(last_modified)[:16]
    ...
    if per_source:
        print("\nSessions by home:")
        for source_key, stats in sorted(per_source.items()):
            print(
                f"  {source_key:16} {stats['sessions']:>5} sessions"
                + (f", {stats['messages']} messages" if stats["messages"] else "")
            )

    if per_agent:
        print("\nSessions by agent:")
        for agent_name, count in sorted(per_agent.items()):
            print(f"  {agent_name:16} {count:>5} sessions")


def get_workspaces_for_project(project_name: str) -> list:
    """Get list of (source_key, workspace) tuples for a project.
    ...
    if project_name not in aliases:
        return []

    sources = aliases[project_name]
    result = []
    for source_key, workspaces in sources.items():
        for workspace in workspaces:
            result.append((source_key, workspace))
    return result


def _list_project_workspaces(project_names: list, output_format: Optional[str] = None):
    """List workspaces belonging to specified projects in ws list format.

    Output format:
        HOME	WORKSPACE
        local	my-project
        wsl:Ubuntu	my-project
    """
    if not project_names:
        return

    # Collect all workspaces from all specified projects
    rows = []
    for project_name in project_names:
        project_workspaces = get_workspaces_for_project(project_name)
        if not project_workspaces:
            sys.stderr.write(f"Error: Project '{project_name}' not found\n")
            continue
        for source_key, workspace in project_workspaces:
            ws_readable = normalize_workspace_name(workspace)
            rows.append((source_key, ws_readable))

    if not rows:
        return

    if output_format == "json":
        data = [{"home": source, "workspace": ws} for source, ws in rows]
        print(json.dumps(data, indent=2))
        return

    # Print in standard ws list format
    print("HOME\tWORKSPACE")
    for source_key, workspace in rows:
        print(f"{source_key}\t{workspace}")
...
def cmd_project_create(args):
    ...
    if alias_name in aliases_data.get("projects", {}):
        sys.stderr.write(f"Error: Project '{alias_name}' already exists\n")
        sys.exit(1)

    aliases_data["projects"][alias_name] = {}

    if save_aliases(aliases_data):
        print(f"Created project '{alias_name}'")
    else:
        sys.exit(1)
...
def cmd_project_delete(args):
    ...
    del aliases_data["projects"][alias_name]

    if save_aliases(aliases_data):
        print(f"Deleted project '{alias_name}'")
    else:
        sys.exit(1)
...
def _add_workspaces_to_alias(
    ...
    if source_key not in aliases_data["projects"][alias_name]:
        aliases_data["projects"][alias_name][source_key] = []

    added = 0
    for workspace in workspaces:
        if workspace not in aliases_data["projects"][alias_name][source_key]:
            aliases_data["projects"][alias_name][source_key].append(workspace)
            added += 1
    return added
...
def _alias_add_all_homes(
    ...
    if save_aliases(aliases_data):
        print(f"Added {total_added} workspace(s) to project '{alias_name}' from all homes")
    else:
        sys.exit(1)
...
def _alias_add_single_source(
    ...
    if save_aliases(aliases_data):
        print(f"Added {added} workspace(s) to project '{alias_name}' [{source_key}]")
    else:
        sys.exit(1)
...
def cmd_project_add(args):
    ...
    if getattr(args, "name_patterns", None):
        raw_workspaces.extend(args.name_patterns)
    use_picker = getattr(args, "pick", False)

    if not raw_workspaces and not use_picker and not getattr(args, "allow_empty", False):
    ...
    if getattr(args, "allow_empty", False) and not raw_workspaces and not use_picker:
...
def cmd_project_remove(args):
    ...
    # If no workspace specified, remove entire project
    if workspace is None:
        cmd_project_delete(SimpleNamespace(name=alias_name))
        return
    ...
    if source_key not in alias_config:
        sys.stderr.write(f"Error: No workspaces from '{source_key}' in project '{alias_name}'\n")
        sys.exit(1)

    target = _match_alias_workspace(alias_config[source_key], workspace)
    if not target:
        sys.stderr.write(
            f"Error: Workspace '{workspace}' not in project '{alias_name}' [{source_key}]\n"
        )
        sys.exit(1)
    ...
    if save_aliases(aliases_data):
        print(f"Removed workspace from project '{alias_name}' [{source_key}]")
    else:
        sys.exit(1)


def cmd_project_config_export(args):
...
def _get_alias_workspace_patterns(alias_config: dict) -> list:
    ...
    Returns full workspace paths to enable exact matching and avoid substring
    collisions (e.g., 'auth' matching 'auth-infra').
    """
    patterns = set()
    for source_key, workspaces in alias_config.items():
        for workspace in workspaces:
            # Convert encoded workspace back to readable path
            readable = normalize_workspace_name(workspace)
            # Use full workspace path for precise matching
            if readable:
                patterns.add(readable)
...
def _collect_non_claude_alias_sessions(
    patterns: list,
    since_date,
    until_date,
    agent: str,
    ...
    Note: patterns should be full workspace paths from alias config for exact matching.
    This prevents substring collisions (e.g., 'auth' matching 'auth-infra').
    """
    source_keys = source_keys or ["local"]
    sessions = []
    seen_files = set()  # Deduplicate across patterns
...
def _print_alias_sessions(sessions: list, output_format: Optional[str] = None):
        ...
        printer.add_row(
            {
                "AGENT": session.get("agent", AGENT_CLAUDE),
                "HOME": session["source"],
                "WORKSPACE": session["workspace_readable"],
...
def cmd_project_lss(
    ...
    alias_config = _filter_alias_config_by_flags(aliases_data["projects"][alias_name], args)
    all_sessions = []
    ...
    if not all_sessions:
        if since_date or until_date:
            ...
            sys.stderr.write(f"No sessions found in project '{alias_name}'{agent_msg}\n")
        return

    output_format = getattr(args, "format", None)
    _print_alias_sessions(all_sessions, output_format=output_format)
...
def cmd_project_export(alias_name: str, output_dir: Union[str, Path], args):
    """Export sessions from all workspaces in an alias."""
    aliases_data = load_aliases()

    if alias_name not in aliases_data.get("projects", {}):
        exit_with_error(f"Project '{alias_name}' not found")
    ...
    if stats["exported"] > 0 or stats["skipped"] > 0:
        print(
            f"\nProject '{alias_name}': {stats['exported']} exported, "
...
def _backup_projects_file() -> Path | None:
    """Create a backup of the projects file. Returns backup path or None if no file to backup."""
    projects_file = get_config_file()
    legacy_projects_file = get_config_dir() / "projects.json"

    target = projects_file if projects_file.exists() else legacy_projects_file
    if not target.exists():
        return None

    backup_path = target.with_suffix(f".backup.{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    try:
        import shutil
...
_backup_aliases_file = _backup_projects_file


def cmd_project_config_import(args):
    """Import projects from JSON file or stdin."""
    import_file = getattr(args, "file", None)
    replace_mode = getattr(args, "replace", False)

    import_data = _load_import_data(import_file)

    # Support both old "aliases" and new "projects" keys
    if "aliases" in import_data and "projects" not in import_data:
        import_data["projects"] = import_data.pop("aliases")
    if "projects" not in import_data:
        sys.stderr.write("Error: Invalid format (missing 'projects' or 'aliases' key)\n")
        sys.exit(1)
    ...
    if replace_mode:
        ...
        for alias_name, sources in import_data["projects"].items():
            if alias_name not in existing_data["projects"]:
                existing_data["projects"][alias_name] = sources
            else:
                _merge_alias_sources(existing_data["projects"][alias_name], sources)
        new_data = existing_data

    if save_aliases(new_data):
        alias_count = len(new_data.get("projects", {}))
...
def _run_metrics_db_migrations(conn: sqlite3.Connection, current_version: int, version_row) -> None:
    ...
    # Migrate from version 5 to 6: add project columns for git-based project identification
    if current_version < DB_VERSION_PROJECT_COLUMNS:
        try:
        ...
        try:
            ...
            pass  # Column already exists
        try:
            conn.execute("ALTER TABLE sessions ADD COLUMN project_short TEXT")
        except sqlite3.OperationalError:
            pass  # Column already exists
        # Create index for project queries
        conn.execute("CREATE INDEX IF NOT EXISTS idx_sessions_project ON sessions(project)")

    # Migrate from version 6 to 7: add home + aggregated metrics tables/columns
    if current_version < DB_VERSION_AGGREGATED_COLUMNS:
        new_columns = [
        ...
        # Create aggregated tables for tools/models
        conn.executescript(
            """
            CREATE TABLE IF NOT EXISTS tool_usage (
                session_id TEXT,
        ...
        # Best-effort backfill for home using existing source values
        try:
...
def _compute_session_aggregates(metrics: dict[str, Any]) -> SessionAggregates:
    """Compute aggregated counts/totals for a session."""
    ...
    for msg in metrics.get("messages", []):
        role = (msg.get("role") or msg.get("type") or "").lower()
        if role == "user":
            agg["user_messages"] += 1
        elif role in ("assistant", "gemini"):
            agg["assistant_messages"] += 1
        ...
        agg["input_tokens"] += int(msg.get("input_tokens", 0) or 0)
        agg["output_tokens"] += int(msg.get("output_tokens", 0) or 0)
        agg["cache_creation_tokens"] += int(msg.get("cache_creation_tokens", 0) or 0)
        agg["cache_read_tokens"] += int(msg.get("cache_read_tokens", 0) or 0)

    tokens_summary = metrics.get("tokens_summary")
    if tokens_summary:
        agg["input_tokens"] += int(tokens_summary.get("input_tokens", 0) or 0)
        agg["output_tokens"] += int(tokens_summary.get("output_tokens", 0) or 0)
        agg["cache_read_tokens"] += int(tokens_summary.get("cache_read_tokens", 0) or 0)
...
def _write_metrics_to_db(
    ...
    session_id = session.get("session_id") or session.get("id") or file_path
    aggregates = _compute_session_aggregates(metrics)
    session_source = session.get("source") or "local"
    session["source"] = session_source
    session_home = session.get("home") or session_source

    session["home"] = session_home
    session["user_messages"] = aggregates["user_messages"]
    session["assistant_messages"] = aggregates["assistant_messages"]
    session["input_tokens"] = aggregates["input_tokens"]
    session["output_tokens"] = aggregates["output_tokens"]
    session["cache_creation_tokens"] = aggregates["cache_creation_tokens"]
    session["cache_read_tokens"] = aggregates["cache_read_tokens"]
    session["first_timestamp"] = aggregates["first_timestamp"]
    session["last_timestamp"] = aggregates["last_timestamp"]

    # If start/end times are missing, derive from message timestamps
    if not session.get("start_time"):
        session["start_time"] = aggregates["first_timestamp"]
    if not session.get("end_time"):
        session["end_time"] = aggregates["last_timestamp"]
    ...
    try:
        ...
        # Tables may not exist yet (older DBs)
        pass
    ...
    conn.execute(
        ...
        INSERT INTO sessions (
            ...
            git_remote_url, project, project_short
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """,
        (
            file_path,
            ...
            session.get("git_remote_url"),
            session.get("project"),
            session.get("project_short"),
        ),
    )
    ...
    # Aggregated tool usage (call counts)
    ...
    if tool_counts:
        try:
            ...
            # Table may not exist for older DBs; skip silently
            pass
    ...
    for msg in metrics.get("messages", []):
        ...
        if not model:
            continue
        rec = model_counts.setdefault(model, {"messages": 0, "tokens": 0})
        rec["messages"] += 1
        rec["tokens"] += (msg.get("input_tokens", 0) or 0) + (msg.get("output_tokens", 0) or 0)
    if model_counts:
        try:
            for model_name, rec in model_counts.items():
                conn.execute(
                    """
                    INSERT INTO model_usage (session_id, model_name, message_count, tokens)
                    VALUES (?, ?, ?, ?)
                """,
                    (session_id, model_name, rec["messages"], rec["tokens"]),
                )
        except sqlite3.OperationalError:
            pass
...
def sync_file_to_db(
    ...
    # Extract git remote info for project identification (local sources only)
    if source == "local":
        workspace_dir = jsonl_file.parent.name
        workspace_path_str = normalize_workspace_name(workspace_dir, verify_local=True)
        workspace_path = Path(workspace_path_str)
        if workspace_path.is_absolute():
            git_remote_url, project, project_short = get_git_remote_for_workspace(workspace_path)
            if git_remote_url:
                session["git_remote_url"] = git_remote_url
                session["project"] = project
                session["project_short"] = project_short
...
def _sync_codex_to_db(
    ...
    label = source_label or "local Codex sessions"
...
def _sync_gemini_to_db(
    ...
    label = source_label or "local Gemini sessions"
    print(f"Scanning {label}...")
            ...
            if distro.get("has_claude"):
                ...
                if wsl_projects:
                    stats = _sync_source_to_db(
                        ...
                        wsl_projects,
                        f"wsl:{distro['name']}",
                        f"WSL ({distro['name']}) Claude sessions",
                        patterns,
                        force,
            ...
            for user in windows_users:
                ...
                if win_projects:
                    stats = _sync_source_to_db(
                        ...
                        f"Windows ({user['username']}) Claude sessions",
                        patterns,
                        force,
                        show_progress,
                    )
...
def cmd_stats_sync(args):
    ...
    if not patterns or patterns == [""]:
        patterns = getattr(args, "workspace", None)
    if not patterns:
        patterns = [""]
    patterns = _expand_patterns_for_sync(patterns)
    jobs_arg = getattr(args, "jobs", None)
    jobs = max(1, int(jobs_arg if jobs_arg is not None else _default_jobs()))
    ...
    stats = _sync_codex_to_db(conn, patterns, force, source_key="local")
    _accumulate_stats(totals, stats)

    # Sync local Gemini sessions (~/.gemini/tmp/)
    stats = _sync_gemini_to_db(conn, patterns, force, source_key="local")
    _accumulate_stats(totals, stats)
...
def _get_stats_workspace_patterns(args) -> list:
    ...
    project_names = getattr(args, "projects", None) or []

    if project_names:
        workspace_patterns.extend([f"@{p}" for p in project_names])
    ...
    # Non-Claude agents don't rely on Claude workspace detection
    if agent in ("codex", "gemini"):
        return []

    _, exists = check_current_workspace_exists()
    if not exists:
        sys.stderr.write(
            "Not in a Claude Code workspace.\n\n"
            "To show stats, either:\n"
            "  • Run from within a workspace directory\n"
            "  • Specify a workspace pattern: stats <pattern>\n"
            "  • Use --aw to include all workspaces: stats --aw\n"
        )
        sys.exit(1)
    ...
    if not this_only and agent in ("auto", "claude"):
        ...
        if alias_name:
            sys.stderr.write(
                f"Using project {alias_name} (use --this for current workspace only)\n"
            )
            return [f"@{alias_name}"]
...
def _expand_alias_to_conditions(alias_name: str, aliases: dict) -> tuple:
    ...
    if alias_name not in aliases:
        print(f" Project '{alias_name}' not found. Treating as workspace pattern.")
    ...
    for source_key, workspaces in aliases[alias_name].items():
        if isinstance(workspaces, list):
            for ws in workspaces:
                # Tie workspace to its home/source to avoid cross-home collisions
                ws_name = get_workspace_name_from_path(ws)
                if source_key == "local":
                    conditions.append(
                        "(s.workspace = ? AND s.source IN ('local','codex','gemini'))"
                    )
                    params.extend([ws_name])
                elif source_key.startswith("wsl:"):
                    conditions.append("(s.workspace = ? AND s.source LIKE ?)")
                    params.extend([ws_name, "wsl:%"])
                elif source_key.startswith("windows"):
                    conditions.append("(s.workspace = ? AND s.source LIKE ?)")
                    params.extend([ws_name, "windows%"])
                elif source_key.startswith("remote:") or "@" in source_key:
                    # Normalize remote sources to match stored form (remote:<hostname>)
                    remote_spec = (
                    ...
                    hostname = remote_spec.split("@")[-1]
                    candidates = [f"remote:{remote_spec}"]
                    if hostname and hostname != remote_spec:
                        candidates.append(f"remote:{hostname}")
                    placeholders = ",".join("?" for _ in candidates)
                    conditions.append(f"(s.workspace = ? AND s.source IN ({placeholders}))")
                    params.extend([ws_name, *candidates])
                else:
                    # local/web/etc.
                    conditions.append("(s.workspace = ? AND s.source = ?)")
                    params.extend([ws_name, source_key])
...
def _build_stats_where_clause(args, workspace_patterns: list) -> tuple:
    ...
    if workspace_patterns:
        ws_conditions = []
        aliases = load_aliases().get("projects", {})
...
def _display_selected_stats(conn, args, where_sql: str, params: list):
    ...
    # New unified --by flag takes precedence
    by_dims = getattr(args, "by", None)
    if by_dims:
        dimensions = [d.strip() for d in by_dims.split(",")]
        display_grouped_stats(
            conn,
            where_sql,
            params,
            dimensions,
    ...
    # Legacy flags (for backward compatibility)
        ...
        # Map legacy --by-workspace [--group X] to new --by format
...
def _print_session_section(session_stats, output_format: str = "tsv", human_readable: bool = False):
    """Print session statistics section."""
    total = session_stats["total_sessions"] or 0
    main = session_stats["main_sessions"] or 0
    agent = session_stats["agent_sessions"] or 0
    messages = session_stats["total_messages"] or 0
    user_messages = session_stats.get("user_messages") or 0
    assistant_messages = session_stats.get("assistant_messages") or 0
    ...
    if output_format == "table":
        ...
        fmt = format_number_human if human_readable else format_number_commas
        print(f"{'Total Sessions':<{label_width}}  {fmt(total):>{value_width}}")
        print(f"{'Main Sessions':<{label_width}}  {fmt(main):>{value_width}}")
        print(f"{'Agent Sessions':<{label_width}}  {fmt(agent):>{value_width}}")
        print(f"{'Total Messages':<{label_width}}  {fmt(messages):>{value_width}}")
        print(f"{'User Messages':<{label_width}}  {fmt(user_messages):>{value_width}}")
        print(f"{'Assistant Msgs':<{label_width}}  {fmt(assistant_messages):>{value_width}}")
    else:
        # TSV format
        print("\nNAME\tVALUE")
        print(f"total_sessions\t{total}")
        print(f"main_sessions\t{main}")
...
def _print_token_section(token_stats, output_format: str = "tsv", human_readable: bool = False):
    ...
    if output_format == "table":
        ...
        value_width = 14
        print()
        print(f"{'Token Metric':<{label_width}}  {'Value':>{value_width}}")
        print(f"{'─' * label_width}  {'─' * value_width}")
        fmt = format_number_human if human_readable else format_number_commas
        print(f"{'Input Tokens':<{label_width}}  {fmt(total_input):>{value_width}}")
        print(f"{'Output Tokens':<{label_width}}  {fmt(total_output):>{value_width}}")
        print(f"{'Cache Created':<{label_width}}  {fmt(cache_creation):>{value_width}}")
        print(f"{'Cache Read':<{label_width}}  {fmt(cache_read):>{value_width}}")
        print(f"{'Cache Hit Ratio':<{label_width}}  {cache_hit_ratio:>{value_width - 1}.1f}%")
    else:
        print("\nNAME\tVALUE")
        print(f"input_tokens\t{total_input}")
        print(f"output_tokens\t{total_output}")
        print(f"cache_created\t{cache_creation}")
        print(f"cache_read\t{cache_read}")
        print(f"cache_hit_ratio\t{cache_hit_ratio:.1f}")


def _print_tool_section(tool_stats, output_format: str = "tsv", human_readable: bool = False):
    """Print tool statistics section."""
    total_tool_uses = tool_stats["total_tool_uses"] or 0
    tool_errors = tool_stats["tool_errors"] or 0
    error_rate = (tool_errors / total_tool_uses * 100) if total_tool_uses > 0 else 0

    if output_format == "table":
        label_width = 16
        value_width = 14
        print()
        print(f"{'Tool Metric':<{label_width}}  {'Value':>{value_width}}")
        print(f"{'─' * label_width}  {'─' * value_width}")
        fmt = format_number_human if human_readable else format_number_commas
        print(f"{'Total Uses':<{label_width}}  {fmt(total_tool_uses):>{value_width}}")
        print(f"{'Errors':<{label_width}}  {fmt(tool_errors):>{value_width}}")
        print(f"{'Error Rate':<{label_width}}  {error_rate:>{value_width - 1}.1f}%")
    else:
        print("\nNAME\tVALUE")
        print(f"total_uses\t{total_tool_uses}")
        print(f"errors\t{tool_errors}")
        print(f"error_rate\t{error_rate:.1f}")


def _print_sources_section(sources):
    """Print homes/sources section (flat format)."""
    if not sources:
        return
    print("\nHOME\tSESSIONS")
    for src in sources:
...
def _print_models_section(models, output_format: str = "tsv", human_readable: bool = False):
    ...
    if output_format == "table":
        ...
        count_width = 14
        print()
        print(f"{'Model':<{model_width}}  {'Messages':>{count_width}}")
        print(f"{'─' * model_width}  {'─' * count_width}")
        fmt = format_number_human if human_readable else format_number_commas
        for model in models:
            model_name = model["model"] or "unknown"
            model_name = truncate_string(model_name, model_width)
            print(f"{model_name:<{model_width}}  {fmt(model['count']):>{count_width}}")
    else:
        print("\nMODEL\tMESSAGES")
        for model in models:
            model_name = model["model"] or "unknown"
            print(f"{model_name}\t{model['count']}")


def _print_workspaces_section(top_workspaces):
    """Print top workspaces section (flat format)."""
    if not top_workspaces:
        return
    print("\nWORKSPACE\tSESSIONS\tMESSAGES")
    for ws in top_workspaces:
        print(f"{ws['name']}\t{ws['sessions']}\t{ws['messages']}")
...
def _print_homes_and_workspaces(
    sources,
    top_workspaces,
    top_limit=None,
    ...
    """Print homes and workspaces together."""
    ...
    for ws in top_workspaces or []:
        home = ws.get("source") or "local"
        rows.append((home, ws["name"], ws["sessions"], ws["messages"]))

    # Apply top limit if set
    if top_limit_int:
        rows = rows[:top_limit_int]

    if output_format == "table":
        # Calculate column widths based on content
        home_width = max(6, max((len(r[0]) for r in rows), default=6))
        ws_width = max(10, max((len(r[1]) for r in rows), default=10))
        # Cap widths to reasonable limits
        home_width = min(home_width, 20)
        ws_width = min(ws_width, 30)
        sess_width = 10
        msg_width = 12

        print(
            f"{'Home':<{home_width}}  {'Workspace':<{ws_width}}  {'Sessions':>{sess_width}}  {'Messages':>{msg_width}}"
        )
        print(f"{'─' * home_width}  {'─' * ws_width}  {'─' * sess_width}  {'─' * msg_width}")
        fmt = format_number_human if human_readable else format_number_commas
        for home, workspace, sessions, messages in rows:
            home_tr = truncate_string(home, home_width)
            ws_tr = truncate_string(workspace, ws_width)
            print(
                f"{home_tr:<{home_width}}  {ws_tr:<{ws_width}}  {fmt(sessions):>{sess_width}}  {fmt(messages):>{msg_width}}"
            )
    else:
        # TSV format
        print("HOME\tWORKSPACE\tSESSIONS\tMESSAGES")
        for home, workspace, sessions, messages in rows:
            print(f"{home}\t{workspace}\t{sessions}\t{messages}")
...
def _print_summary_stats(stats: SummaryStatsData):
    ...
    human_readable = stats.human_readable

    _print_homes_and_workspaces(
        stats.sources,
        stats.top_workspaces,
        top_limit=stats.top_limit,
        output_format=output_format,
        human_readable=human_readable,
    )
    _print_session_section(stats.session_stats, output_format, human_readable)
    _print_token_section(stats.token_stats, output_format, human_readable)
    _print_tool_section(stats.tool_stats, output_format, human_readable)
    _print_models_section(stats.models, output_format, human_readable)
    if stats.time_stats is not None:
        _print_time_summary(stats.time_stats, include_breakdown=False, output_format=output_format)
...
def display_tool_stats(conn: sqlite3.Connection, where_sql: str, params: list):
    """Display tool usage statistics (flat format)."""
    cursor = conn.execute(
        f"""
        SELECT
            t.tool_name,
    ...
    print("TOOL\tUSES\tERRORS\tERROR_PCT")
    for row in tools:
        uses = row["uses"]
        errors = row["errors"]
        error_pct = (errors / uses * 100) if uses > 0 else 0
        print(f"{row['tool_name']}\t{uses}\t{errors}\t{error_pct:.1f}")


def display_model_stats(conn: sqlite3.Connection, where_sql: str, params: list):
    """Display model usage statistics (flat format)."""
    cursor = conn.execute(
        f"""
        SELECT
            m.model,
    ...
    print("MODEL\tMESSAGES\tSESSIONS\tAVG_OUTPUT")
    for row in models:
        model = row["model"] or "unknown"
        messages = row["messages"]
        avg_output = (row["output_tokens"] / messages) if messages > 0 else 0
        print(f"{model}\t{messages}\t{row['sessions']}\t{avg_output:.0f}")
...
def display_workspace_stats(
    conn: sqlite3.Connection, where_sql: str, params: list, group_by: Optional[str] = None
):
    """Display per-workspace statistics with alias aggregation.

    Args:
        group_by: None for full breakdown (home + agent), "home" to aggregate by home,
                  "agent" to aggregate by agent
    """
    # Build SQL query based on grouping
    if group_by == "home":
        # Aggregate by workspace + source (home), sum across agents
        query = f"""
        ...
        # Aggregate by workspace + agent, sum across homes
        query = f"""
            SELECT s.workspace, s.agent, COUNT(*) as sessions, SUM(s.message_count) as messages,
                   SUM(m_stats.input_tokens) as input_tokens, SUM(m_stats.output_tokens) as output_tokens
            FROM sessions s
            LEFT JOIN (SELECT session_id, SUM(input_tokens) as input_tokens, SUM(output_tokens) as output_tokens
                       FROM messages GROUP BY session_id) m_stats ON s.session_id = m_stats.session_id
            WHERE {where_sql}
            GROUP BY s.workspace, s.agent ORDER BY sessions DESC
        """
    else:
        # Default: full breakdown by workspace + source + agent
        query = f"""
            SELECT s.workspace, s.source, s.agent, COUNT(*) as sessions, SUM(s.message_count) as messages,
                   SUM(m_stats.input_tokens) as input_tokens, SUM(m_stats.output_tokens) as output_tokens
            FROM sessions s
            LEFT JOIN (SELECT session_id, SUM(input_tokens) as input_tokens, SUM(output_tokens) as output_tokens
                       FROM messages GROUP BY session_id) m_stats ON s.session_id = m_stats.session_id
            WHERE {where_sql}
            GROUP BY s.workspace, s.source, s.agent ORDER BY sessions DESC
        """
    ...
    aliases_data = load_aliases()
    workspace_to_alias = _build_workspace_to_alias_map(aliases_data.get("projects", {}))
    ...
    if group_by == "home":
        alias_stats, unaliased = _aggregate_workspace_stats_detailed(
            all_workspaces, workspace_to_alias
        )
        if alias_stats:
            print("\nAliases")
        ...
        print(f"{'Workspace':<30} {'Home':<10} {'Sessions':>8} {'Messages':>10}")
        print("-" * DISPLAY_SEPARATOR_WIDTH)
        ...
        # For agent grouping, aggregate across homes
        unaliased = []
        for row in all_workspaces:
            ws_name, agent = row["workspace"], row["agent"]
            sessions, messages = row["sessions"], row["messages"] or 0
            unaliased.append(
                {"name": ws_name, "agent": agent, "sessions": sessions, "messages": messages}
            )

        print(f"\nIndividual Workspaces (Top {MAX_TOP_WORKSPACES})")
        print(f"{'Workspace':<30} {'Agent':<10} {'Sessions':>8} {'Messages':>10}")
        print("-" * DISPLAY_SEPARATOR_WIDTH)
        ...
        for row in unaliased[:MAX_TOP_WORKSPACES]:
            ...
            print(f"{ws_name:<30} {row['agent']:<10} {row['sessions']:>8,} {row['messages']:>10,}")

    else:
        # Default: full breakdown with home + agent columns
        unaliased = []
        for row in all_workspaces:
            ws_name = row["workspace"]
            source = row["source"]
        ...
        print(f"\nIndividual Workspaces (Top {MAX_TOP_WORKSPACES})")
        print(f"{'Workspace':<26} {'Home':<10} {'Agent':<8} {'Sessions':>8} {'Messages':>10}")
        print("-" * DISPLAY_SEPARATOR_WIDTH)

        for row in unaliased[:MAX_TOP_WORKSPACES]:
            ws_name = row["name"][:24] if len(row["name"]) > 24 else row["name"]
            print(
                f"{ws_name:<26} {row['source']:<10} {row['agent']:<8} "
                f"{row['sessions']:>8,} {row['messages']:>10,}"
            )
...
def format_duration_dhm(seconds: float) -> str:
    """Format seconds as Xd Xh Xm for human display."""
    if seconds < SECONDS_PER_MINUTE:
...
def format_duration_hours(seconds: float) -> str:
    """Format seconds as decimal hours (e.g., 143.2h)."""
    if seconds < SECONDS_PER_MINUTE:
        return f"{int(seconds)}s"
    hours = seconds / 3600
    return f"{hours:.1f}h"
...
def format_number_human(n: int) -> str:
    """Format number with K/M/B suffixes for human display."""
    if n < 1000:
        return str(n)
    if n < 1_000_000:
...
def format_number_commas(n: int) -> str:
...
def _print_daily_breakdown(
    ...
    """Print daily breakdown table (flat format)."""
    print("\nDATE\tWORK_SECONDS\tPERIODS\tMESSAGES")

    for date_str in sorted(daily_stats.keys(), reverse=True):
        stats = daily_stats[date_str]
        work_seconds = stats["work_seconds"]
        print(f"{date_str}\t{work_seconds}\t{stats['work_periods']}\t{stats['messages']}")
...
def _print_time_summary(
    time_stats: dict, include_breakdown: bool = False, output_format: str = "tsv"
):
    ...
    if output_format == "table":
        ...
        value_width = 14
        print()
        print(f"{'Time Metric':<{label_width}}  {'Value':>{value_width}}")
        print(f"{'─' * label_width}  {'─' * value_width}")
        print(f"{'Total Work':<{label_width}}  {format_duration_hours(total_work):>{value_width}}")
        print(
            f"{'Work Periods':<{label_width}}  {format_number_commas(total_periods):>{value_width}}"
        )
        print(f"{'Session Files':<{label_width}}  {format_number_commas(num_files):>{value_width}}")
        if first_date:
            print(f"{'First Date':<{label_width}}  {first_date:>{value_width}}")
        if last_date:
            print(f"{'Last Date':<{label_width}}  {last_date:>{value_width}}")
    else:
        print("\nNAME\tVALUE")
        print(f"total_work_seconds\t{total_work}")
        print(f"work_periods\t{total_periods}")
        print(f"session_files\t{num_files}")
        if first_date:
...
def display_time_stats(conn: sqlite3.Connection, where_sql: str, params: list):
    """Display time tracking statistics with daily breakdown (flat format)."""
    time_stats = _compute_time_stats(conn, where_sql, params)
    _print_time_summary(time_stats, include_breakdown=True)


def display_grouped_stats(
    conn: sqlite3.Connection,
    where_sql: str,
    params: list,
    dimensions: list[str],
    ...
    """Display stats grouped by specified dimensions.

    Args:
        dimensions: List of dimension names (home, agent, workspace, day, model, tool)
        limit: Maximum rows to display
        output_format: 'table', 'tsv', or 'json'. None = auto-detect (table for TTY, tsv for pipes)
        human_readable: Use K/M/B for numbers and Xd Xh Xm for time

    Output formats:
    - table: Fixed-width columns, human-readable (like df, ps)
    - tsv: Tab-separated values for piping to other tools
    ...
    # Validate dimensions
    invalid = [d for d in dimensions if d not in STATS_DIMENSIONS]
    if invalid:
        valid = ", ".join(STATS_DIMENSIONS.keys())
        print(f"Error: Invalid dimensions: {', '.join(invalid)}", file=sys.stderr)
        print(f"Valid dimensions: {valid}", file=sys.stderr)
        sys.exit(1)

    # Determine which tables we need to join
    requires_messages = any(STATS_DIMENSIONS[d].get("requires_messages") for d in dimensions)
    requires_tools = any(STATS_DIMENSIONS[d].get("requires_tools") for d in dimensions)

    # Build SELECT clause for dimensions
    dim_selects = [str(STATS_DIMENSIONS[d]["sql"]) + f" as {d}" for d in dimensions]

    # Build metrics based on what tables are available
    # Always join messages table to get token counts
    if requires_tools:
        ...
        metric_labels = ["Uses", "Errors"]
        metric_types = ["number", "number"]
    else:
        # Standard metrics with token counts, cache tokens, and work time
        # Always join messages to get accurate token sums
        metrics = [
            "COUNT(DISTINCT s.file_path) as sessions",
            "SUM(s.message_count) as messages",
            "COALESCE(CAST(SUM(s.work_period_seconds) AS INTEGER), 0) as work_secs",
            "COALESCE(SUM(m_tokens.input_tokens), 0) as input_tokens",
            "COALESCE(SUM(m_tokens.output_tokens), 0) as output_tokens",
            "COALESCE(SUM(m_tokens.cache_creation_tokens), 0) as cache_write",
            "COALESCE(SUM(m_tokens.cache_read_tokens), 0) as cache_read",
        ]
        metric_labels = [
            "Sessions",
            "Messages",
        ...
        metric_types = ["number", "number", "time", "number", "number", "number", "number"]

    # Build FROM and JOIN clauses
    from_clause = "sessions s"
    if requires_tools:
        from_clause = "tool_uses t LEFT JOIN sessions s ON t.file_path = s.file_path"
        # If we also need messages (e.g., for model dimension), join messages table too
        if requires_messages:
            from_clause += " LEFT JOIN messages m ON t.file_path = m.file_path"
    elif requires_messages or "model" in dimensions:
        from_clause += " LEFT JOIN messages m ON s.file_path = m.file_path"
        # Add token aggregation subquery
        from_clause += """
            LEFT JOIN (
                SELECT file_path,
                       SUM(input_tokens) as input_tokens,
                ...
                FROM messages GROUP BY file_path
            ) m_tokens ON s.file_path = m_tokens.file_path"""
    else:
        # Add token aggregation subquery for non-message queries
        from_clause += """
            LEFT JOIN (
                SELECT file_path,
                       SUM(input_tokens) as input_tokens,
    ...
    group_by = ", ".join(str(STATS_DIMENSIONS[d]["sql"]) for d in dimensions)

    # Build ORDER BY clause (by first metric descending)
    order_by = "2 DESC" if len(dimensions) == 1 else f"{len(dimensions) + 1} DESC"

    # Construct full query
    select_clause = ", ".join(dim_selects + metrics)
    query = f"""
    ...
    # Compute totals across all matching rows (no LIMIT) using the same joins
    if requires_tools:
        ...
        totals = [totals_row["uses"] or 0, totals_row["errors"] or 0]
    else:
        totals_row = conn.execute(
            f"""
            SELECT
        ...
        totals = [
            totals_row["sessions"] or 0,
            totals_row["messages"] or 0,
            totals_row["work_secs"] or 0,
            totals_row["input_tokens"] or 0,
            totals_row["output_tokens"] or 0,
            totals_row["cache_write"] or 0,
            totals_row["cache_read"] or 0,
        ]
    ...
    header_labels = [str(STATS_DIMENSIONS[d]["label"]).upper() for d in dimensions]
    header_labels.extend([label.upper() for label in metric_labels])
    ...
    for row in rows:
        ...
        for d in dimensions:
            value = row[d] or "(none)"
            transform = STATS_DIMENSIONS[d].get("transform")
            if transform == "short_name" and value and value != "(none)":
                value = get_folder_short_name(value)
            row_data[d] = value
            row_values.append(value)

        # Process metric columns (use values as returned; totals already computed)
        for i, label in enumerate(metric_labels):
            metric_value = row[i + len(dimensions)] or 0
            row_data[label.lower()] = metric_value
            row_values.append(metric_value)

        data_rows.append({"data": row_data, "values": row_values})

    # Output based on format
    if output_format == "json":
        _output_grouped_stats_json(data_rows, totals, dimensions, metric_labels)
    elif output_format == "table":
        _output_grouped_stats_table(
            data_rows,
            totals,
            header_labels,
            dimensions,
        ...
        _output_grouped_stats_tsv(data_rows, totals, header_labels, dimensions, metric_labels)


def _output_grouped_stats_tsv(
    data_rows: list, totals: list, headers: list, dimensions: list, metric_labels: list
):
    """Output stats in TSV format (tab-separated values)."""
    print("\t".join(headers))

    for row in data_rows:
        row_parts = [str(v) for v in row["values"]]
        print("\t".join(row_parts))

    # Total row
    total_parts = ["total"] + [""] * (len(dimensions) - 1)
    total_parts.extend([str(t) for t in totals])
    print("\t".join(total_parts))


def _output_grouped_stats_json(
    data_rows: list, totals: list, dimensions: list, metric_labels: list
):
    """Output stats in JSON format."""
    output = {
        "rows": [row["data"] for row in data_rows],
        "totals": {label.lower(): totals[i] for i, label in enumerate(metric_labels)},
    }
    print(json.dumps(output, indent=2))


def _output_grouped_stats_table(
    data_rows: list,
    totals: list,
    headers: list,
    dimensions: list,
    ...
    """Output stats in human-readable table format with fixed-width columns."""
    # Define column widths (cast to int for type checker)
    dim_widths: dict[str, int] = {d: int(STATS_DIMENSIONS[d].get("width", 20)) for d in dimensions}

    # Metric column widths (right-justified numbers)
    if human_readable:
        metric_widths = [12] * len(metric_labels)  # Wider for human format
    else:
        metric_widths = [12] * len(metric_labels)

    # Format a metric value based on type and human_readable flag
    def fmt_metric(value: int, mtype: str) -> str:
        if mtype == "time":
            if human_readable:
                return format_duration_dhm(value)
            return str(value)
        else:  # number
            if human_readable:
                return format_number_human(value)
            return format_number_commas(value)
    ...
    for i, label in enumerate(metric_labels):
        ...
        header_parts.append(f"{headers[len(dimensions) + i]:>{width}}")
    print("  ".join(header_parts))

    # Separator line
    sep_parts = []
    for d in dimensions:
        sep_parts.append("─" * dim_widths[d])
    ...
    for row in data_rows:
        ...
        # Dimension columns (left-justified, truncated)
        for i, d in enumerate(dimensions):
            width = dim_widths[d]
            value = truncate_string(str(values[i]), width)
            row_parts.append(f"{value:<{width}}")

        # Metric columns (right-justified)
        for i in range(len(metric_labels)):
            width = metric_widths[i]
            raw_value = values[len(dimensions) + i]
            mtype = metric_types[i] if i < len(metric_types) else "number"
            formatted = fmt_metric(raw_value, mtype)
    ...
    total_parts = []
    total_label = "TOTAL"
    total_parts.append(f"{total_label:<{dim_widths[dimensions[0]]}}")
    for i in range(1, len(dimensions)):
        total_parts.append(" " * dim_widths[dimensions[i]])
    for i in range(len(metric_labels)):
        width = metric_widths[i]
        mtype = metric_types[i] if i < len(metric_types) else "number"
        formatted = fmt_metric(totals[i], mtype)
        total_parts.append(f"{formatted:>{width}}")
    print("  ".join(total_parts))
...
def _scan_codex_gemini_sessions(
    ...
    # Prefer the shortest pattern to maximize substring matches across encodings (e.g., basename)
    pattern_str: str = cast(str, sorted(patterns, key=len)[0]) if patterns else ""
...
def _show_no_sessions_message(agent: str, location: str):
    """Show message when no sessions are found."""
    if agent == "auto":
        sys.stderr.write(f"No sessions found in {location}\n")
    else:
        sys.stderr.write(f"No {agent} sessions found in {location}\n")
...
def _list_windows_sessions(args, patterns: list, since_date, until_date):
    ...
    if not all_sessions:
        _show_no_sessions_message(agent, "Windows")
        return

    source_label = f"Windows ({username})" if username else "Windows"
    print_sessions_output(
        all_sessions,
        source_label,
        workspaces_only,
        output_format=getattr(args, "format", None),
        legacy_format=getattr(args, "legacy_format", False),
    )
...
def _list_wsl_sessions(args, patterns: list, since_date, until_date):
    ...
    if not all_sessions:
        _show_no_sessions_message(agent, f"WSL '{distro_name}'")
        return
...
def _list_remote_workspaces_only(
    remote_host: Optional[str],
    patterns: list,
    agent: str = "auto",
    output_format: Optional[str] = None,
    ...
    # Deduplicate across agents and collect unique workspaces
    seen = set()
    unique_workspaces = []
    for ws_info in sorted(all_workspaces, key=lambda x: x["decoded"]):
        ...
        if decoded in seen:
            continue
        seen.add(decoded)
        unique_workspaces.append(decoded)

    # Use TablePrinter for consistent output
    home_label = f"remote:{remote_host}" if remote_host else "remote"
    columns = ["HOME", "WORKSPACE", "SESSIONS", "STATUS", "LAST_MODIFIED"]
    printer = TablePrinter(columns, output_format=output_format)
        ...
        printer.add_row(
            {
                "HOME": home_label,
                "WORKSPACE": ws,
                "SESSIONS": "-",  # Not available for remote workspace-only listing
                "STATUS": "ok",  # Can't check remote existence
                "LAST_MODIFIED": "",
            }
        )
    printer.print()
...
def _list_ssh_remote_sessions(args, patterns: list, since_date, until_date):
    ...
    if not check_ssh_connection(remote_host):
        if os.environ.get("AGENT_HISTORY_HOME"):
            sys.stderr.write(f"Warning: Cannot connect to {remote_host} via passwordless SSH\n")
    ...
    if workspaces_only:
        _list_remote_workspaces_only(remote_host, patterns, agent, output_format=output_format)
    else:
        sessions = _collect_remote_session_details(
            remote_host, patterns, since_date, until_date, agent
        )
        if not sessions:
            sys.stderr.write(f"No sessions found on remote '{remote_host}'\n")
            return
        print_sessions_output(
            sessions,
    ...
    if not sessions:
        if not workspaces_only:
            ...
            if since_date or until_date:
                ...
                sys.stderr.write(f"No sessions found matching '{pattern_str}'\n")
        return
...
def _get_batch_windows_sessions(args, patterns: list, since_date, until_date) -> list:
    ...
    if not windows_projects_dir:
        ...
        sys.stderr.write("  • Try: agent-history home --windows\n")
        sys.exit(1)
    ...
    if not sessions:
        # No data available in Windows path; mark lenient so callers don't fail hard
        args.lenient = True
        return []
...
def _get_batch_wsl_sessions(args, patterns: list, since_date, until_date) -> list:
    ...
    for scan_agent in agents_to_scan:
        if scan_agent == AGENT_CLAUDE:
            ...
            if not wsl_projects_dir:
                if agent == AGENT_CLAUDE:
                    ...
                    sys.stderr.write("  • Try: agent-history home --wsl\n")
                    sys.exit(1)
                continue
            sessions.extend(
        ...
        if not sessions_dir or not sessions_dir.exists():
            if agent == scan_agent:
                sys.stderr.write(f"Error: Cannot access WSL distribution '{distro_name}'\n")
                sys.stderr.write("\nTips:\n")
                sys.stderr.write(f"  • Make sure {scan_agent} is installed in WSL\n")
    ...
    if not sessions:
        sys.stderr.write(f"No sessions found in WSL '{distro_name}'\n")
        return []
...
def _get_export_output_path(
    ...
    Args:
        session: Session dict with 'file' and 'workspace' keys
        ts_prefix: Timestamp prefix for filename (optional)
        output_dir: Output directory path
        flat: If True, use flat structure without workspace subdirs
        remote_host: Remote host name for source tagging
        extension: File extension (default: ".md", use ".ndjson" for JSON export)
    ...
    if flat:
        output_name = (
            f"{ts_prefix}_{jsonl_file.stem}{extension}"
            if ts_prefix
            else f"{jsonl_file.stem}{extension}"
        )
        return output_dir / output_name, output_name

    source_tag = get_source_tag(remote_host)
    output_name = (
        f"{source_tag}{ts_prefix}_{jsonl_file.stem}{extension}"
        if ts_prefix
        else f"{source_tag}{jsonl_file.stem}{extension}"
    )
...
def _export_single_session(
    ...
    """Export a single session to markdown or NDJSON file(s).

    Args:
        jsonl_file: Source JSON/JSONL file path
        output_file: Destination file path (.md or .ndjson)
        output_name: Name of the output file
        agent_type: Agent type (claude, codex, gemini)
        messages: Pre-read messages
        minimal: If True, produce minimal output without metadata
        split_lines: Line threshold for splitting (None to disable, markdown only)
        quiet: If True, suppress per-file output
        include_source: If True, copy source JSON/JSONL alongside export
        export_json: If True, export as NDJSON instead of markdown
    """
    try:
        if export_json:
            # Export as unified NDJSON
            records = parse_to_unified_ndjson(jsonl_file, agent_type, messages)
            workspace = _infer_workspace_for_unified(jsonl_file, agent_type)
            header = _build_unified_header(agent_type, workspace)
            write_unified_ndjson(records, output_file, quiet, header=header)
        # Codex/Gemini files don't support splitting yet; use direct export
        elif agent_type in (AGENT_CODEX, AGENT_GEMINI):
            _write_single_file(
                jsonl_file,
                output_file,
                minimal,
        ...
        # Copy source JSON/JSONL if requested
        if include_source:
            _copy_source_json(jsonl_file, output_file, output_name, quiet)

    except (OSError, json.JSONDecodeError) as e:
        sys.stderr.write(f"Error converting {jsonl_file.name}: {e}\n")


def _copy_source_json(jsonl_file: Path, output_file: Path, output_name: str, quiet: bool) -> None:
    """Copy the source JSON/JSONL file alongside the markdown export.

    Args:
        jsonl_file: Source JSON/JSONL file path
        output_file: Destination markdown file path (used to determine output dir)
        output_name: Base name for output file
        quiet: If True, suppress output
    """
    import shutil

    # Determine output path: same directory as markdown, same base name + source extension
    base_name = output_name.rsplit(".md", 1)[0]
    json_output = output_file.parent / f"{base_name}{jsonl_file.suffix}"

    try:
        shutil.copy2(jsonl_file, json_output)
        if not quiet:
            size_kb = json_output.stat().st_size / 1024
            print(f"{json_output} ({size_kb:.1f} KB)")
    except (OSError, shutil.Error) as e:
        sys.stderr.write(f"Error copying {jsonl_file.name}: {e}\n")


def _export_sessions_to_markdown(sessions: list, args, output_dir: Path):
    """Export sessions to markdown or NDJSON files.

    Args:
        sessions: List of session dictionaries
        args: Parsed arguments (for force, flat, minimal, split, remote, include_source, export_json)
        output_dir: Output directory path
    """
    ...
    extension = ".ndjson" if export_json else ".md"

    if not quiet and total_sessions:
        format_name = "NDJSON" if export_json else "markdown"
        sys.stderr.write(f"Exporting {total_sessions} sessions to {format_name}...\n")
    ...
    def export_one(session: dict) -> None:
        ...
        output_file, output_name = _get_export_output_path(
            session, ts_prefix, output_dir, flat, remote_host, extension
        )
...
def _print_all_homes_results(
    all_results, workspaces_only, use_uris: bool = True, output_format: Optional[str] = None
):
    """Print results from all homes listing.

    Args:
        all_results: List of (source_label, sessions) tuples
        workspaces_only: If True, only show unique workspaces with counts
        use_uris: If True, show workspace URIs (default). If False, show legacy format.
        output_format: 'tsv', 'json', or None (table)
    """
    if not all_results:
    ...
    if workspaces_only:
        ...
        for source_label, sessions in all_results:
            ...
            for ws in sorted(workspaces.keys()):
                ...
                status = _check_workspace_status(ws, source_label)
                last_mod = (
                    data["last_modified"].strftime("%Y-%m-%d %H:%M")
        ...
        columns = ["HOME", "WORKSPACE", "SESSION", "MESSAGES", "SIZE", "MODIFIED"]
        printer = TablePrinter(columns, output_format=output_format)
...
def _source_label_to_home(source_label: str) -> str:
    """Convert source label to short home identifier.

    Examples:
        "Local (WSL)" -> "local"
        "Windows (kvsan)" -> "windows:kvsan"
        "Remote (vm01)" -> "remote:vm01"
        "Web (claude.ai)" -> "web"
    """
    ...
    if label.startswith("windows"):
        # Extract username from "Windows (username)"
        if "(" in source_label and ")" in source_label:
            user = source_label.split("(")[1].split(")")[0]
    ...
    if label.startswith("web"):
        return "web"
    # WSL distro names like "WSL (Ubuntu)"
    if label.startswith("wsl"):
        if "(" in source_label and ")" in source_label:
            distro = source_label.split("(")[1].split(")")[0]
            return f"wsl:{distro}"
        return "wsl"
...
def cmd_list_all_homes(args):
    """List workspaces or sessions from all homes: local + configured sources."""
    in_wsl = is_running_in_wsl()
    workspaces_only = getattr(args, "workspaces_only", False)
    agent = getattr(args, "agent", None) or "auto"
    no_wsl = getattr(args, "no_wsl", False)
    ...
    saved_sources = get_saved_homes()

    # Categorize sources
    windows_sources = [s for s in saved_sources if s.startswith("windows:")]
    wsl_sources = [s for s in saved_sources if s.startswith("wsl:")]
    ssh_remotes = [s for s in saved_sources if "@" in s]

    # Add any explicitly passed remotes
    explicit_remotes = getattr(args, "remotes", []) or []
    for r in explicit_remotes:
        if r not in ssh_remotes:
            ssh_remotes.append(r)

    all_results = []

    # 1. Always collect local sessions
    all_results.extend(_collect_local_sessions(patterns, since_date, until_date, in_wsl, agent))

    # 2. Collect from configured Windows sources (only if in WSL)
    if in_wsl and not no_windows and windows_sources:
        all_results.extend(
    ...
    # 3. Collect from configured WSL sources (only if on Windows)
    if not in_wsl and not no_wsl and wsl_sources:
        all_results.extend(
    ...
    # 4. Collect from SSH remotes
    if not no_remote:
        all_results.extend(
            _collect_all_remote_sessions(ssh_remotes, patterns, since_date, until_date, agent)
        )

    output_format = getattr(args, "format", None)
    _print_all_homes_results(all_results, workspaces_only, output_format=output_format)


def _lsh_get_local() -> Optional[dict]:
    """Get local home info."""
    try:
        projects_dir = get_claude_projects_dir()
...
def _lsh_get_wsl() -> list:
    """Get WSL distributions info (when on Windows)."""
    results = []
    try:
...
def _lsh_get_windows() -> list:
    """Get Windows users info (when on WSL)."""
    results = []
    try:
        windows_users = get_windows_users_with_claude()
        for user in windows_users:
            results.append(
                {
                    "home": f"windows:{user['username']}",
                    "path": user["path"],
                    "workspaces": user["workspace_count"],
                }
            )
    except (OSError, PermissionError):
        pass
...
def _lsh_get_configured() -> list:
    """Get configured homes (SSH remotes, Windows, WSL)."""
    sources = get_saved_homes()
    results = []
    for source in sources:
        results.append({"home": source, "path": "-", "workspaces": "-"})
    return results


def _lsh_get_web() -> Optional[dict]:
    """Get Claude.ai web session info."""
    token = get_access_token()
    org_uuid = get_org_uuid_from_claude_config()

    if token and org_uuid:
        try:
        ...
        except WebSessionsError as e:
            return {"home": "web", "path": "-", "workspaces": 0, "status": f"error: {e}"}
    elif not token:
        return {"home": "web", "path": "-", "workspaces": 0, "status": "not authenticated"}
    elif not org_uuid:
        return {"home": "web", "path": "-", "workspaces": 0, "status": "not authenticated"}
    return None
...
def _get_lsh_display_flags(args) -> tuple:
    ...
    Returns:
        Tuple of (show_local, show_wsl, show_windows, show_web, show_remotes)
    """
    show_web_flag = getattr(args, "web", False)
    no_filters = (
        not args.wsl
        and not args.windows
        and not show_web_flag
        and not getattr(args, "remotes", False)
    )
    show_local = args.local or (no_filters and not args.local)
    show_remotes = getattr(args, "remotes", False) or (
        not args.local and not args.wsl and not args.windows and not show_web_flag
    )
    show_web = show_web_flag or no_filters
    return show_local, args.wsl, args.windows, show_web, show_remotes


def cmd_lsh(args):
    """List all home folders with Claude Code installations."""
    home_verb = getattr(args, "home_verb", None)
    lsh_action = getattr(args, "lsh_action", None)

    if lsh_action == "add" or home_verb == "add":
        cmd_lsh_add(args)
        return
    if lsh_action == "remove" or home_verb == "remove":
        cmd_lsh_remove(args)
        return
    if lsh_action == "clear":
        cmd_lsh_clear(args)
    ...
    if home_verb == "show":
        ...
        return
    if home_verb == "export":
        _cmd_home_export(args)
        return
    if home_verb == "stats":
        _cmd_home_stats(args)
        return

    homes = _gather_homes(args)

    # Print using TablePrinter
    if homes:
        output_format = getattr(args, "format", None)
        columns = ["HOME", "PATH", "WORKSPACES"]
        printer = TablePrinter(columns, output_format=output_format)
        for home in homes:
            printer.add_row(
                {
...
def _resolve_wsl_distro(distro_arg: Optional[str]) -> Optional[str]:
    """Resolve WSL distro name from argument or auto-detect.

    Args:
        distro_arg: Distro name or "auto" to auto-detect

    Returns:
        Distro name or None if not found
    """
    if distro_arg and distro_arg != "auto":
        return distro_arg

    # Auto-detect: find first WSL distro with Claude data
    wsl_distros = get_wsl_distributions()
    claude_distros = [
        d for d in wsl_distros if d.get("has_claude") or d.get("has_codex") or d.get("has_gemini")
    ]
    if claude_distros:
        return claude_distros[0]["name"]

    # Fall back to any WSL distro
    if wsl_distros:
        return wsl_distros[0]["name"]

        return None


def _resolve_windows_user() -> Optional[str]:
    """Resolve Windows username for adding Windows home.

    Returns:
        Windows username or None if not found
    """
    if not is_running_in_wsl():
        sys.stderr.write("Error: --windows flag only works from WSL\n")
        return None

    # Find Windows users with Claude data
    windows_users = get_windows_users_with_claude()
    if windows_users:
        # Each entry is a dict with 'username' key
        return windows_users[0]["username"]

    sys.stderr.write("Error: No Windows users with Claude data found\n")
    return None


def _gather_homes(args) -> list[dict]:
    """Collect homes based on filter flags."""
    in_wsl = is_running_in_wsl()
    show_wsl = getattr(args, "wsl", False)
    show_windows = getattr(args, "windows", False)
    show_web_flag = getattr(args, "web", False)
    show_local_flag = getattr(args, "local", False)
    show_remotes_flag = getattr(args, "remotes", False)

    specific_filter = (
        show_wsl or show_windows or show_web_flag or show_local_flag or show_remotes_flag
    )

    homes = []
    if show_local_flag or not specific_filter:
        local_info = _lsh_get_local()
        if local_info:
            homes.append(local_info)

    if show_wsl:
        homes.extend(_lsh_get_wsl())
    if show_windows and in_wsl:
        homes.extend(_lsh_get_windows())

    if show_web_flag or not specific_filter:
        web_info = _lsh_get_web()
        if web_info:
            homes.append(web_info)

    if show_remotes_flag or not specific_filter:
        homes.extend(_lsh_get_configured())
    return homes
...
def cmd_lsh_add(args):
    ...
    add_windows = getattr(args, "windows", False)
    add_wsl = getattr(args, "wsl", None)
    add_web = getattr(args, "web", False)

    # Determine what to add
    if add_web:
        source = "web"
    elif add_windows:
        user = _resolve_windows_user()
        if not user:
            sys.exit(1)
        source = f"windows:{user}"
    elif add_wsl is not None:
        distro = _resolve_wsl_distro(add_wsl)
        if not distro:
            sys.stderr.write("Error: No WSL distributions found\n")
            sys.exit(1)
        source = f"wsl:{distro}"
    elif not source:
        sys.stderr.write("Error: Specify a source (user@host), --windows, --wsl, or --web\n")
        sys.exit(1)
        ...
        sys.stderr.write("Expected: user@hostname, --windows, --web, or --wsl\n")
        sys.exit(1)
    ...
    if source in homes:
        print(f"Home '{source}' already configured.")
        return
...
def cmd_lsh_remove(args):
    ...
    source = getattr(args, "source", None)
    remove_windows = getattr(args, "windows", False)
    remove_wsl = getattr(args, "wsl", None)
    remove_web = getattr(args, "web", False)
    ...
    if remove_web:
        ...
        windows_sources = [s for s in homes if s.startswith("windows:")]
        if not windows_sources:
            sys.stderr.write("Error: No Windows home configured\n")
            sys.exit(1)
        source = windows_sources[0]
    elif remove_wsl is not None:
        # Find and remove matching wsl:* source
        if remove_wsl == "auto":
            wsl_sources = [s for s in homes if s.startswith("wsl:")]
            if not wsl_sources:
                sys.stderr.write("Error: No WSL home configured\n")
                sys.exit(1)
            source = wsl_sources[0]
        else:
            source = f"wsl:{remove_wsl}"
    elif not source:
        sys.stderr.write("Error: Specify a source to remove, --windows, --web, or --wsl\n")
        sys.exit(1)

    if source not in homes:
        sys.stderr.write(f"Error: Home '{source}' not found.\n")
        sys.stderr.write(f"Configured homes: {', '.join(homes) if homes else '(none)'}\n")
        sys.exit(1)
...
def _cmd_home_show(args):
    """Show a single home entry with summary details."""
    ...
    homes = _gather_homes(all_args)
    matches = [h for h in homes if h.get("home") == args.name]
    if not matches:
        exit_with_error(f"Home '{args.name}' not found.")

    home = matches[0]
    agent = getattr(args, "agent", None) or "auto"
    summary = _summarize_home(home["home"], agent)

    output_format = getattr(args, "format", None)
    if output_format in (None, "table"):
        print(f"Home: {home['home']}")
        if summary["type"]:
            print(f"Type: {summary['type']}")
        if summary["host"]:
            print(f"Host: {summary['host']}")
        status = summary["status"] or home.get("status") or "unknown"
        print(f"Status: {status}")
        print()
        print(f"Workspaces: {home.get('workspaces', '-')}")
        print(f"Sessions: {summary['sessions']}")
        if summary["last_modified"]:
            print(f"Last Modified: {summary['last_modified'].strftime('%Y-%m-%d %H:%M')}")
        if summary["top_workspaces"]:
            print()
            print("Top Workspaces:")
            for ws, count in summary["top_workspaces"]:
                print(f"  {ws:20} {count} sessions")
        ...
        rows = [
            ...
            ("Status", summary["status"] or home.get("status") or "unknown"),
            ("Workspaces", str(home.get("workspaces", "-"))),
            ("Sessions", str(summary["sessions"])),
            (
                "Last Modified",
                summary["last_modified"].strftime("%Y-%m-%d %H:%M")
                if summary["last_modified"]
                else "",
            ),
        ]
        ...
        out = {
            ...
            "host": summary["host"],
            "status": summary["status"] or home.get("status") or "unknown",
            "workspaces": home.get("workspaces"),
            "sessions": summary["sessions"],
            "last_modified": summary["last_modified"].isoformat()
            if summary["last_modified"]
            else None,
            "top_workspaces": [
                {"workspace": ws, "sessions": count} for ws, count in summary["top_workspaces"]
            ],
        }
        print(json.dumps(out, indent=2))


def _cmd_home_export(args):
    """Export all sessions from selected homes."""
    export_args = SimpleNamespace(**vars(args))
    export_args.target = []
    export_args.all_workspaces = True
    export_args.all_homes = getattr(args, "all_homes", False)
    _apply_home_flags_from_names(export_args, getattr(args, "names", []) or [])
    _dispatch_export(export_args)


def _cmd_home_stats(args):
    """Show stats for selected homes."""
    stats_args = SimpleNamespace(**vars(args))
    stats_args.workspace = []
    stats_args.all_workspaces = True
    stats_args.all_homes = getattr(args, "all_homes", False)
    _apply_home_flags_from_names(stats_args, getattr(args, "names", []) or [])
    _dispatch_stats(stats_args)


def _summarize_home(home_name: str, agent: str = "auto") -> dict:
    """Summarize sessions for a given home."""
    result = {
        "type": None,
        "host": None,
        "status": None,
    ...
    # Determine type/host from name
    if home_name == "local":
        result["type"] = "local"
        result["host"] = platform.node()
    elif home_name.startswith("wsl:"):
        result["type"] = "wsl"
        result["host"] = home_name.split(":", 1)[1]
    elif home_name.startswith("windows"):
        result["type"] = "windows"
    elif home_name == "web":
        result["type"] = "web"
    elif home_name.startswith("remote:"):
        result["type"] = "ssh"
        result["host"] = home_name.split(":", 1)[1]
    elif "@" in home_name:
        result["type"] = "ssh"
        result["host"] = home_name
    ...
    try:
        if home_name == "local":
            sessions = collect_sessions_with_dedup(
                [""], include_cached=True, skip_message_count=True, agent=agent
            )
            result["status"] = "ok"
        elif home_name.startswith("remote:") or "@" in home_name:
            remote = home_name.split(":", 1)[1] if home_name.startswith("remote:") else home_name
            remote_result = _collect_remote_sessions(remote, [""], None, None, agent)
            if remote_result:
                _, sessions = remote_result
                result["status"] = "ok"
            else:
                result["status"] = "unreachable"
        elif home_name.startswith("wsl:"):
            distro = home_name.split(":", 1)[1]
            target_agent = agent if agent != "auto" else AGENT_CLAUDE
            sessions_dir = get_agent_wsl_dir(distro, target_agent)
            if sessions_dir and sessions_dir.exists():
                if target_agent == AGENT_CLAUDE:
                    sessions = collect_sessions_with_dedup(
                        [""],
                        projects_dir=sessions_dir,
            ...
            user = home_name.split(":", 1)[1] if ":" in home_name else None
            target_agent = agent if agent != "auto" else AGENT_CLAUDE
            sessions_dir = get_agent_windows_dir(user, target_agent)
            if sessions_dir and sessions_dir.exists():
                if target_agent == AGENT_CLAUDE:
                    sessions = collect_sessions_with_dedup(
                        [""],
            ...
            org_uuid = get_org_uuid_from_claude_config()
            if token and org_uuid:
                web_sessions = fetch_web_sessions(token, org_uuid)
                result["sessions"] = len(web_sessions)
        ...
        # Gracefully degrade if inaccessible
        result["status"] = result.get("status") or "unreachable"

    if sessions:
        result["sessions"] = len(sessions)
        # Compute last modified and per-workspace counts
        last_mod = None
        counts: dict[str, int] = {}
        for s in sessions:
            ws = s.get("workspace_readable") or s.get("workspace") or ""
            counts[ws] = counts.get(ws, 0) + 1
            mod = s.get("modified")
            if mod and (last_mod is None or mod > last_mod):
                last_mod = mod
        result["last_modified"] = last_mod
        result["top_workspaces"] = sorted(counts.items(), key=lambda kv: kv[1], reverse=True)[:3]
    return result
...
def _get_reset_items(what: Optional[str], config_dir: Path) -> list:
    ...
    if reset_all or what in ("aliases", "projects"):
        # Check both new and legacy file names
        projects_file = config_dir / "projects.json"
        aliases_file = config_dir / "aliases.json"
        if projects_file.exists():
            items.append(("Projects", projects_file))
        if aliases_file.exists():
            items.append(("Legacy aliases", aliases_file))
...
def cmd_gemini_index(args):
    """Manage Gemini hash→path index: list mappings or add paths."""
...
def _filter_fetch_workspaces(workspaces: list, pattern: str) -> list:
    ...
    if not pattern or pattern.lower() in ("", "*", "all"):
        return workspaces
    pattern_lower = pattern.lower()
    filtered = [ws for ws in workspaces if pattern_lower in ws.lower()]
    if not filtered:
...
def _validate_wsl_windows_mutex(args):
    ...
    # Historically this exited, but allow combining flags so guards and patterns
    # can resolve scope (tests expect success when a pattern is provided).
    return not (wsl_flag and windows_flag)
...
def _resolve_remote_flag(args):
    ...
    if wsl_flag:
        ...
        agent = getattr(args, "agent", None) or "auto"
        wsl_distros = get_wsl_distributions()
        if not wsl_distros and os.environ.get("AGENT_HISTORY_HOME"):
            # In isolated/test environments without WSL available, skip hard exit so
            # cross-home guard can surface the expected pattern/project error instead.
            return None
        if wsl_distros:
...
def _add_lsw_parser(subparsers):
    """Add ws (list workspaces) subparser."""
    ...
    parser.add_argument(
        "--web",
        action="store_true",
        help="List sessions from Claude.ai web (requires authentication)",
    )
    parser.add_argument(
        "--local",
        action="store_true",
    ...
    parser.add_argument(
        ...
        dest="all_homes",
        action="store_true",
        help="List workspaces from all homes (local + WSL/Windows + web + remotes)",
    )
    parser.add_argument(
        "--agent",
...
def _add_lss_parser(subparsers):
    """Add ss (list sessions) subparser."""
    parser = subparsers.add_parser(
        "ss",
        help="List sessions",
    ...
    parser.add_argument(
        "--no-web",
        action="store_true",
        help="Exclude web sessions (applies to --ah)",
    )
    parser.add_argument(
        "--since",
        metavar="DATE",
    ...
    parser.add_argument(
        ...
        dest="all_homes",
        action="store_true",
        help="List sessions from all homes (local + WSL/Windows + web + remotes)",
    )
    parser.add_argument(
        "--alias",
        metavar="NAME",
...
def _add_export_parser(subparsers):
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="all_homes",
        help="Export from all homes: local + WSL + Windows + web + remotes (default: local only)",
    )
    parser.add_argument(
        "-r",
        "--remote",
    ...
    parser.add_argument(
        "--web", action="store_true", help="Export from Claude.ai web (requires authentication)"
    )
    ...
    parser.add_argument("--no-web", action="store_true", help="Skip web sessions")
    parser.add_argument(
        "--local",
        action="store_true",
...
def _add_project_parser(subparsers):
    """Add project subparser with aliases."""
    parser = subparsers.add_parser(
        "project",
        help="Manage workspace projects",
        description="Create and manage workspace projects for consolidated exports",
    )
    # Add hidden aliases (for backward compat)
    for alias in ["projects", "alias"]:
        subparsers._name_parser_map[alias] = parser

    project_subparsers = parser.add_subparsers(
        dest="project_command", help="Project subcommand", metavar="COMMAND"
    )

    # Helper function for adding subparsers
    def add_subparser(name, **kwargs):
        return project_subparsers.add_parser(name, **kwargs)

    # list
    parser_list = add_subparser(
        "list",
    ...
    # show
    parser_show = add_subparser(
        "show", help="Show project details", description="Show details of a single project"
    )
    parser_show.add_argument("name", help="Project name")
    _add_output_format(parser_show)

    # create - deprecated, use 'add' instead (which auto-creates)
    parser_create = add_subparser(
        "create",
        help="(deprecated: use 'add')",
        description="Deprecated: use 'add' which auto-creates",
    )
    parser_create.add_argument("name", help="Project name")

    # delete
    parser_delete = add_subparser(
        "delete",
        help="Delete a project",
        description="Delete a project and all its workspace associations",
    )
    parser_delete.add_argument("name", help="Project name")

    # add
    parser_add = add_subparser(
        "add",
        help="Add workspace(s) to project",
        description="Add one or more workspaces to a project",
    )
    parser_add.add_argument(
        "name",
        nargs="?",
        default="",
        help="Project name (default: auto-detect from current directory)",
    )
    parser_add.add_argument(
    ...
    # remove
    parser_remove = add_subparser(
        "remove",
        help="Remove workspace from project",
        description="Remove a workspace from a project",
    )
    parser_remove.add_argument("name", help="Project name")
    ...
    add_subparser(
        ...
        description="Export projects configuration to JSON (stdout)",
    )

    # import
    parser_import = add_subparser(
        "import",
        help="Import projects from JSON",
        description="Import projects from JSON file or stdin",
    )
    parser_import.add_argument("file", nargs="?", help="JSON file to import (default: stdin)")
    parser_import.add_argument(
        "--replace", action="store_true", help="Replace all projects instead of merging"
    )
...
def _add_stats_parser(subparsers):
    ...
    parser.add_argument(
        ...
        default=None,
        help="Output format: table (human-readable), tsv (tab-separated), json. Default: table for TTY, tsv for pipes",
    )
    parser.add_argument(
        "-H",
        "--human",
    ...
    # Legacy flags (deprecated, use --by instead)
    parser.add_argument(
        "--tools",
        action="store_true",
        help=argparse.SUPPRESS,  # Hidden, use --by tool
    )
    parser.add_argument(
        "--models",
        action="store_true",
        help=argparse.SUPPRESS,  # Hidden, use --by model
    )
    parser.add_argument(
        "--by-workspace",
        action="store_true",
        help=argparse.SUPPRESS,  # Hidden, use --by workspace
    )
    parser.add_argument(
        "--group",
        choices=["home", "agent"],
        help=argparse.SUPPRESS,  # Hidden, use --by
    )
    parser.add_argument(
        "--by-day",
        action="store_true",
        help=argparse.SUPPRESS,  # Hidden, use --by day
    )
    parser.add_argument(
        "--source",
        metavar="SOURCE",
...
def _add_reset_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "reset",
        help="Reset stored data (database, settings, projects)",
        description="Delete metrics database, settings, and/or projects.",
    )

    parser.add_argument(
        "what",
        nargs="?",
        choices=["db", "settings", "projects", "aliases", "all"],
        default="all",
        help="What to reset: db (metrics), settings (SSH remotes), projects, or all (default: all)",
    )
    parser.add_argument("-y", "--yes", action="store_true", help="Skip confirmation prompt")
    return parser


def _add_home_scope_flags(parser):
    ...
    parser.add_argument("--no-remote", action="store_true", help="Exclude SSH remotes (with --ah)")
    parser.add_argument("--no-web", action="store_true", help="Exclude web sessions (with --ah)")
    parser.add_argument(
        "--local",
        action="store_true",
...
def _add_workspace_scope_flags(parser, positional_name: str = "workspace"):
    """Add workspace scope flags."""
    parser.add_argument(
        positional_name,
        nargs="*",
        help="Workspace path(s) (exact)",
    )
    parser.add_argument(
        "-n",
        "--name",
...
def _add_export_options(parser):
    ...
    _add_date_filters(parser)


def _add_stats_options(parser):
    """Add stats options."""
    parser.add_argument(
        "--sync",
        action="store_true",
        help="Force sync before showing stats (always syncs unless --no-sync is set)",
    )
    parser.add_argument(
        "--no-sync",
        action="store_true",
...
def _create_argument_parser():
    ...
    # Create subparsers for commands (use custom parser class for better error messages)
    subparsers = parser.add_subparsers(
        dest="command",
        help="Command to execute",
        required=False,
        parser_class=ChoiceHintArgumentParser,
        metavar="COMMAND",
    )
    ...
    home_add.add_argument(
        "--wsl", nargs="?", const="auto", metavar="DISTRO", help="Add WSL as a home"
    )
    ...
    home_remove.add_argument("--wsl", nargs="?", const="auto", metavar="DISTRO", help="Remove WSL")
    ...
    _add_export_options(home_export)
    _add_home_scope_flags(home_export)
    ...
    _add_stats_options(home_stats)
    _add_home_scope_flags(home_stats)
    ...
    _add_workspace_scope_flags(ws_list)
    _add_home_scope_flags(ws_list)
    _add_output_format(ws_list)
    ...
    _add_export_options(ws_export)
    _add_home_scope_flags(ws_export)
    ws_export.add_argument("--aw", "--all-workspaces", dest="all_workspaces", action="store_true")
    ws_export.add_argument("--project", action="append", dest="projects", metavar="NAME")

    ws_stats = ws_sub.add_parser("stats", help="Stats for workspace")
    ws_stats.set_defaults(command="ws", ws_verb="stats")
    _add_workspace_scope_flags(ws_stats)
    _add_stats_options(ws_stats)
    _add_home_scope_flags(ws_stats)
    ...
    _add_workspace_scope_flags(sess_list, positional_name="workspace")
    _add_home_scope_flags(sess_list)
    _add_date_filters(sess_list)
    ...
    _add_home_scope_flags(sess_show)
    _add_workspace_scope_flags(sess_show, positional_name="workspace")
    _add_date_filters(sess_show)
    ...
    _add_workspace_scope_flags(sess_export, positional_name="target")
    ...
    _add_export_options(sess_export)
    _add_home_scope_flags(sess_export)

    sess_stats = sess_sub.add_parser("stats", help="Stats for sessions")
    sess_stats.set_defaults(command="session", session_verb="stats")
    _add_workspace_scope_flags(sess_stats)
    _add_stats_options(sess_stats)
    _add_home_scope_flags(sess_stats)
    ...
    proj_add.add_argument("-n", "--name", dest="name_patterns", action="append")
    proj_add.add_argument("--pick", action="store_true", help="Interactive workspace picker")
    proj_add.add_argument(
        "--allow-empty",
        action="store_true",
        help=argparse.SUPPRESS,  # Hidden flag used for legacy alias create compatibility
    )
    proj_add.add_argument(
        "-r", "--remote", metavar="HOST", help="Add workspace from remote host (user@hostname)"
    )
    proj_add.add_argument("--wsl", action="store_true", help="Add workspace from WSL")
    proj_add.add_argument("--windows", action="store_true", help="Add workspace from Windows")
    proj_add.add_argument(
        "--ah",
        "--all-homes",
        action="store_true",
    ...
    proj_remove.add_argument("workspace", nargs="?", help="Workspace to remove (optional)")
    proj_remove.add_argument(
        "-r", "--remote", metavar="HOST", help="Remove workspace from remote source"
    )
    proj_remove.add_argument("--wsl", action="store_true", help="Remove workspace from WSL source")
    proj_remove.add_argument(
        "--windows", action="store_true", help="Remove workspace from Windows source"
    )
    ...
    _add_export_options(proj_export)
    _add_home_scope_flags(proj_export)
    ...
    _add_stats_options(proj_stats)
    _add_home_scope_flags(proj_stats)

    # Legacy config import/export kept as hidden subcommands
    proj_conf_export = proj_sub.add_parser(
        "config-export",
        help=argparse.SUPPRESS,
        description="Export projects configuration to JSON (stdout)",
    )
    proj_conf_export.set_defaults(command="project", project_command="config-export")

    proj_conf_import = proj_sub.add_parser(
        "config-import",
        help=argparse.SUPPRESS,
        description="Import projects configuration from JSON",
    )
    proj_conf_import.set_defaults(command="project", project_command="config-import")
    proj_conf_import.add_argument("file", nargs="?", help="JSON file to import (default: stdin)")
    proj_conf_import.add_argument(
        "--replace", action="store_true", help="Replace all projects instead of merging"
    )
...
def _add_gemini_index_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "gemini-index",
        help="Manage Gemini hash→path index",
        description=(
...
def _combine_workspace_inputs(
    workspaces: Optional[list],
    name_patterns: Optional[list],
    all_flag: bool,
    default_all: bool = False,
) -> list[str]:
    """Combine exact workspace paths and name patterns into a single list.

    Exact paths are prefixed with '=' to force exact matching; name patterns
    are used as-is for substring matching.
    """
    combined: list[str] = []
    for ws in workspaces or []:
        if ws is None:
            continue
        # Preserve project patterns (@project) to allow alias expansion downstream
        if isinstance(ws, str) and ws.startswith("@"):
            combined.append(ws)
        else:
            combined.append(f"={ws}")
    if name_patterns:
        combined.extend([p for p in name_patterns if p])

    if all_flag and not combined:
        combined.append("")
    elif default_all and not combined:
        combined.append("")
    return combined


def _apply_home_flags_from_names(args, names: list[str]):
    """Apply --home selections by mutating args flags/remotes."""
    if not names:
        return
    remotes = list(getattr(args, "remotes", None) or [])
    for home_name in names:
        if home_name == "local":
            ...
            args.wsl = True
            remotes.append(f"wsl://{home_name.split(':', 1)[1]}")
        elif home_name.startswith("windows:"):
            args.windows = True
            remotes.append(f"windows:{home_name.split(':', 1)[1]}")
        elif home_name == "wsl":
            args.wsl = True
        elif home_name == "windows":
            args.windows = True
        else:
            remotes.append(home_name)
    args.remotes = remotes


def _apply_project_sources_to_args(args, project_name: str) -> None:
    """Mutate args with home/remote flags based on project sources."""
    aliases = load_aliases().get("projects", {})
    project = aliases.get(project_name)
    if not project:
        return

    remotes = list(getattr(args, "remotes", None) or [])
    # Track if we need WSL/Windows sources
    windows_flag = getattr(args, "windows", False)
    wsl_flag = getattr(args, "wsl", False)

    for source_key in project.keys():
        if source_key.startswith("remote:") or "@" in source_key:
            host = source_key.split(":", 1)[1] if source_key.startswith("remote:") else source_key
            if host not in remotes:
                remotes.append(host)
        elif source_key.startswith("wsl:"):
            # Use wsl:// scheme for sync helper
            distro = source_key.split(":", 1)[1]
            target = f"wsl://{distro}"
            ...
            # Use windows:<user> if provided
            target = source_key if ":" in source_key else "windows"
            if target not in remotes:
                remotes.append(target)
            windows_flag = True

    args.remotes = remotes
    args.windows = windows_flag
    args.wsl = wsl_flag
...
def _print_lsw_results(
    all_results: list, workspaces_only: bool, output_format: Optional[str] = None
):
    """Print lsw results using TablePrinter with HOME, WORKSPACE, SESSIONS, STATUS, LAST_MODIFIED."""
    if not all_results:
        return
    ...
    if workspaces_only:
        ...
        for source_label, sessions in all_results:
            ...
            workspaces = _aggregate_workspaces(sessions)
            for ws in sorted(workspaces.keys()):
                ws_data = workspaces[ws]
                status = _check_workspace_status(ws, source_label)
                last_mod = (
                    ws_data["last_modified"].strftime("%Y-%m-%d %H:%M")
                    if ws_data["last_modified"]
                    else ""
                )
                printer.add_row(
                    {
...
def _dispatch_lsw_additive(args):
    ...
    _print_lsw_results(all_results, workspaces_only, output_format=getattr(args, "format", None))


def _dispatch_ws(args):
    """Dispatch workspace (ws) commands."""
    verb = getattr(args, "ws_verb", None) or "list"
    _apply_home_flags_from_names(args, getattr(args, "homes", []) or [])
    workspace_inputs = getattr(args, "workspace", []) or []
    name_patterns = getattr(args, "name_patterns", []) or []
    output_format = getattr(args, "format", None)

    project_names = getattr(args, "projects", None) or []
    project_only = False
    if project_names:
        # For ws list --project, list workspaces belonging to the project
        _list_project_workspaces(project_names, output_format=output_format)
        project_only = not (
            workspace_inputs or name_patterns or getattr(args, "all_workspaces", False)
        )

    if verb == "show":
        _dispatch_ws_show(args, workspace_inputs, name_patterns)
        return
    ...
    if (
        verb == "list"
        and not workspace_inputs
        and not name_patterns
        and not getattr(args, "all_workspaces", False)
        and not getattr(args, "this_only", False)
    ):
        pattern, exists = check_current_workspace_exists()
        if exists:
            alias_for_ws = get_alias_for_workspace(pattern, "local")
            if alias_for_ws:
                sys.stderr.write(
                    f"Using project {alias_for_ws} (use --this for current workspace only)\n"
                )
                _list_project_workspaces([alias_for_ws], output_format=output_format)
                return

    combined = _combine_workspace_inputs(
        workspace_inputs,
        name_patterns,
        getattr(args, "all_workspaces", False),
        default_all=True,
    )
    args.pattern = combined
    _dispatch_lsw(args)


def _dispatch_ws_show(args, workspace_inputs: list[str], name_patterns: list[str]) -> None:
    """Show workspace summary and recent sessions."""
    combined = _combine_workspace_inputs(
        workspace_inputs,
        name_patterns,
        getattr(args, "all_workspaces", False),
    ...
    if not combined:
        exit_with_error("No workspace specified for ws show.")
    # We only show the first resolved workspace to align with spec (single workspace)
    pattern = combined[0]
    # Reuse session listing to gather data
    list_args = SimpleNamespace(**vars(args))
    list_args.workspace = [pattern]
    list_args.patterns = [pattern]
    list_args.workspaces_only = False
    list_args.name_patterns = []
    list_args.all_workspaces = False
    list_args.since_date = getattr(args, "since_date", None)
    list_args.until_date = getattr(args, "until_date", None)
    list_args.agent = getattr(args, "agent", None) or "auto"

    sessions = collect_sessions_with_dedup(
        list_args.patterns,
        since_date=list_args.since_date,
        until_date=list_args.until_date,
        include_cached=True,
        agent=list_args.agent,
    )
    if not sessions:
        exit_with_error("No sessions found for the specified workspace.")

    # Infer workspace/home from first session
    first = sessions[0]
    workspace_path = first.get("workspace_readable") or first.get("workspace", "")
    home_label = first.get("source") or "local"
    ...
    for s in sessions:
        try:
            ...
            continue
    last_modified = max((s.get("modified") for s in sessions if s.get("modified")), default=None)

    # Recent sessions (top 3 by modified desc)
    sorted_sessions = sorted(
        sessions,
        key=lambda s: s.get("modified") or datetime.min,
        reverse=True,
    )
    recent = sorted_sessions[:3]

    # Output human-readable format
    print(f"Workspace: {get_folder_short_name(workspace_path) or workspace_path}")
    ...
    if last_modified:
        print(f"Last Modified: {last_modified.strftime('%Y-%m-%d %H:%M')}")
    print()
    print("Recent Sessions:")
    for s in recent:
        mod = s.get("modified")
        mod_str = mod.strftime("%Y-%m-%d %H:%M") if mod else ""
        fname = s.get("filename") or s.get("file") or ""
        msg_count = _format_message_count(s)
        print(f"  {fname:20}  {msg_count:>4} messages  {mod_str}")
...
def _dispatch_lsw(args):
    ...
    remotes = getattr(args, "remotes", None) or []
    agent = getattr(args, "agent", None) or "auto"
    show_web = getattr(args, "web", False)
    all_homes = getattr(args, "all_homes", False)
    all_workspaces_flag = getattr(args, "all_workspaces", False)

    # Guard: require project or explicit pattern for cross-home access
    has_windows = getattr(args, "windows", False)
    has_wsl = getattr(args, "wsl", False)
    explicit_patterns = [p for p in patterns if p and p.lower() not in _MATCH_ALL_PATTERNS]
    needs_cross_home = bool(has_windows or has_wsl or remotes or all_homes)
    if needs_cross_home and not all_workspaces_flag and not explicit_patterns:
        if has_windows:
            target = "Windows"
        elif has_wsl:
            target = "WSL"
            ...
            target = "remote"
        _require_project_or_pattern_for_cross_home([], target)

    output_format = getattr(args, "format", None)

    # If --web alone (without --all-homes), only show web sessions
    if show_web and not all_homes:
        _dispatch_lsw_web(output_format=output_format)
        return
    ...
    if all_homes:
        ...
        # Also include web sessions with --all-homes
        _dispatch_lsw_web(output_format=output_format)
...
def _dispatch_lsw_web(output_format: Optional[str] = None):
    """List web workspaces using TablePrinter.
    ...
    try:
            ...
            printer.add_row(
                {
                    "HOME": "web",
                    "WORKSPACE": ws,
                    "SESSIONS": workspace_counts[ws],
                    "STATUS": "-",  # Status not applicable for web
                    "LAST_MODIFIED": "-",
                }
            )
        ...
        # Silently skip if web auth not available
        pass
...
def _dispatch_lss_to_alias(alias_name: str, args):
    ...
    agent = getattr(args, "agent", None) or "auto"
    cmd_project_lss(
        alias_name, since_date=since_date, until_date=until_date, agent=agent, args=args
    )


def _require_project_or_pattern_for_cross_home(workspace_list: list, target: str) -> None:
    """Exit with error if accessing non-local home without project or explicit pattern.

    Only applies when there's a local workspace that would be implicitly matched.
    If no local workspace exists, the user is explicitly accessing remote - allow it.

    Args:
        workspace_list: Explicit workspace patterns from command line
        target: Description of the target (e.g., "remote", "Windows", "WSL")
    """
    if workspace_list:
        return  # Explicit pattern provided, allow access

    pattern, exists = check_current_workspace_exists()
    if not exists:
        return  # No local workspace context, no implicit matching would happen

    alias_for_ws = get_alias_for_workspace(pattern, "local")
    if not alias_for_ws:
        sys.stderr.write(f"Error: Cannot access {target} without a project or explicit pattern.\n")
        sys.stderr.write(
            f"The current workspace is not part of a project that includes {target} workspaces.\n"
        )
        sys.stderr.write("Options:\n")
        sys.stderr.write("  - Use --aw to include all workspaces\n")
        sys.stderr.write("  - Use -n <pattern> to specify a workspace pattern\n")
        sys.stderr.write("  - Define a project that ties local and remote workspaces together\n")
        sys.exit(1)
...
def _dispatch_lss_local_additive(args, workspace_list: list, remotes: list):
    ...
    # Guard: require project or explicit pattern for cross-home access
    remote_desc = f"remote ({', '.join(remotes)})" if len(remotes) <= 2 else "remotes"
    _require_project_or_pattern_for_cross_home(workspace_list, remote_desc)
...
def _dispatch_lss_all_homes(args, workspace_list: list):
    ...
    # Guard: require project or explicit pattern for cross-home access
    if not all_workspaces_flag:
        _require_project_or_pattern_for_cross_home(workspace_list, "all homes")
    ...
    if alias_for_ws:
        sys.stderr.write(f"Using project {alias_for_ws} (use --this for current workspace only)\n")
        cmd_project_lss(alias_for_ws, since_date=since_date, until_date=until_date, agent=agent)
        return
...
def _dispatch_lss_local(args, workspace_list: list):
    ...
    has_windows = getattr(args, "windows", False)
    has_wsl = getattr(args, "wsl", False)
    needs_remote_scope = bool(remote_value or has_windows or has_wsl)

    # Guard: require project or explicit pattern for cross-home access
    if needs_remote_scope and not all_workspaces_flag:
        if has_windows:
            target = "Windows"
        elif has_wsl:
            target = "WSL"
        else:
            target = f"remote ({remote_value})"
        _require_project_or_pattern_for_cross_home(workspace_list, target)
...
def _dispatch_lss(args):
    ...
    workspace_list = []
    for raw in workspace_inputs:
        coerced = _coerce_target_to_workspace_pattern(raw)
        if isinstance(raw, str) and raw.startswith("="):
            workspace_list.append(raw)
        else:
            workspace_list.append(coerced)
    name_patterns = getattr(args, "name_patterns", None) or []
    if name_patterns:
        workspace_list.extend(name_patterns)
    if getattr(args, "all_workspaces", False) and not workspace_list:
    ...
    args.workspace = workspace_list
    args.patterns = workspace_list
    ...
    if getattr(args, "web", False) and getattr(args, "no_web", False):
        exit_with_error("Conflicting flags: --web and --no-web")

    local_flag = getattr(args, "local", False)
    remotes = getattr(args, "remotes", None) or []

    project_names = getattr(args, "projects", None) or []
    handled_project = False
    for project_name in project_names:
        _dispatch_lss_to_alias(project_name, args)
        handled_project = True
    ...
    if handled_project and not workspace_list and not getattr(args, "all_workspaces", False):
        return
    ...
    # If --web alone (without --all-homes), only show web sessions
    if show_web and not all_homes:
        _dispatch_lss_web(args)
        return

    # Handle --all-homes
    if all_homes:
        _dispatch_lss_all_homes(args, workspace_list)
        # Also include web sessions with --all-homes (unless --no-web)
        if not no_web:
            _dispatch_lss_web(args)
    else:
...
def _dispatch_lss_web(args, print_header: bool = False):
    """List web sessions from Claude.ai (resolved to local paths when possible).

    Args:
        args: Command arguments
        print_header: If True, print column headers. Set False when appending to other output.
    """
    try:
        token, org_uuid = resolve_web_credentials()
        sessions = fetch_web_sessions(token, org_uuid)
        ...
        # Build map of GitHub repos to local workspace paths
        projects_dir = get_claude_projects_dir()
        github_map = build_github_to_workspace_map(projects_dir) if projects_dir else {}
        ...
        for session in sessions:
            session_id = session.get("uuid", session.get("id", "unknown"))
            workspace = get_web_session_workspace(session, github_map) or "(no repo)"
            created_raw = session.get("created_at") or session.get("createdAt") or ""
            msg_count_val = session.get("message_count")
            if msg_count_val is None:
                msg_count_val = session.get("messageCount")
            msg_count = msg_count_val if msg_count_val is not None else "?"
        ...
        # TSV without headers when appending to other output (legacy expectation)
        if not print_header and output_format in (None, "tsv"):
            for row in rows:
                print(
                    f"{row['AGENT']}\t{row['HOME']}\t{row['WORKSPACE']}\t{row['FILE']}\t{row['MESSAGES']}\t{row['DATE']}"
                )
            return
        ...
        # Silently skip if web auth not available (e.g., not on macOS)
        pass
...
def _resolve_session_file_for_show(session_id: str, args) -> Path:
    """Resolve a session identifier to a local JSONL file."""
    candidate = Path(session_id).expanduser()
    if candidate.exists():
        return candidate

    patterns = _combine_workspace_inputs(
        getattr(args, "workspace", []) or [],
        getattr(args, "name_patterns", []) or [],
        getattr(args, "all_workspaces", False),
    )
    since_date = parse_date_with_validation(getattr(args, "since", None), "--since")
    until_date = parse_date_with_validation(getattr(args, "until", None), "--until")
    ...
    for session in sessions:
        ...
        filename = session.get("filename") or path_obj.name
        if session_id == filename or session_id in str(path_obj):
            matches.append(path_obj)
            continue
        if session_id in path_obj.name:
            matches.append(path_obj)

    if not matches:
        exit_with_error(f"Session '{session_id}' not found in current scope.")
    ...
    if len(unique_matches) > 1:
        ...
        exit_with_error(f"Session '{session_id}' matched multiple files:\n{details}")

    return unique_matches[0]


def _dispatch_session_show(args):
    """Show a single session as markdown."""
    _apply_home_flags_from_names(args, getattr(args, "homes", []) or [])
    session_path = _resolve_session_file_for_show(args.session_id, args)
    agent_type = detect_agent_from_path(session_path)
    try:
        markdown = _parse_file_to_markdown(session_path, agent_type)
    except (OSError, json.JSONDecodeError) as e:
        exit_with_error(f"Error reading session: {e}")
...
def _dispatch_export_single_file(args, jsonl_file: str):
    ...
    if agent_type != AGENT_CLAUDE or not split_lines:
        ...
        # Copy source JSONL if requested (for local files only)
        if include_source and not final_remote:
            _copy_source_json(Path(jsonl_file), output_file, output_file.name, quiet)
        return
    ...
    if final_remote:
        ...
        # Copy source JSONL if requested
        if include_source:
            _copy_source_json(Path(jsonl_file), output_file, output_file.name, quiet)
...
def _dispatch_export_all_homes(args, output_dir: Union[str, Path], patterns: list, remotes: list):
    ...
    # In isolated/test environments (AGENT_HISTORY_HOME), skip cross-platform sources to avoid
    # slow or unavailable validations.
    if os.environ.get("AGENT_HISTORY_HOME"):
        args.no_wsl = True
        args.no_windows = True
        args.no_remote = True
        args.no_web = getattr(args, "no_web", False) or False
    ...
    export_config = ExportAllConfig(
        ...
        split=args.split,
        agent=getattr(args, "agent", None) or "auto",
        jobs=jobs_arg if jobs_arg is not None else _default_jobs(),
        quiet=getattr(args, "quiet", False),
        no_wsl=getattr(args, "no_wsl", False),
        no_windows=getattr(args, "no_windows", False),
        no_remote=getattr(args, "no_remote", False),
...
def _resolve_export_targets(args, targets: list, output_dir: Union[str, Path]) -> tuple:
    ...
    if not this_only and agent in ("auto", "claude"):
        ...
        if alias_for_ws:
            ...
            cmd_project_export(alias_for_ws, output_dir, args)
            return [], True
...
def _route_export(args, output_dir: Union[str, Path], final_patterns: list, final_remotes):
    ...
    # If --web alone (without --all-homes), only export web sessions
    if show_web and not all_homes:
        _dispatch_export_web(args, output_dir)
        return

    # Guard: require project or explicit pattern for cross-home access
    raw_targets = args.target if hasattr(args, "target") and args.target else []
    name_patterns = getattr(args, "name_patterns", None) or []
    explicit_patterns = raw_targets + name_patterns
    needs_cross_home = bool(final_remotes or has_windows or has_wsl or all_homes)
    if needs_cross_home and not all_workspaces and not explicit_patterns:
        if has_windows:
            target = "Windows"
        elif has_wsl:
            target = "WSL"
            ...
            target = (
                f"remote ({', '.join(final_remotes[:2])})" if len(final_remotes) <= 2 else "remotes"
            )
        _require_project_or_pattern_for_cross_home([], target)
    ...
    if local_flag and final_remotes:
        ...
        final_remote = final_remotes[0] if final_remotes else None
        cmd_batch(_build_export_config(args, output_dir, final_patterns, final_remote))


def _export_single_web_session(
    token: str,
    org_uuid: str,
    session_id: str,
    web_subdir: Path,
    ...
    """Export a single web session. Returns True on success."""
    try:
        session_data = fetch_web_session(token, org_uuid, session_id)
        jsonl_content = web_session_to_jsonl(session_data)
        if not jsonl_content.strip():
            return False

        temp_jsonl = web_subdir / f"{session_id}.jsonl"
        temp_jsonl.write_text(jsonl_content, encoding="utf-8")

        markdown = parse_jsonl_to_markdown(temp_jsonl, minimal=minimal)
        md_file = web_subdir / f"{session_id}.md"
        ...
        if not quiet:
            sys.stderr.write(f"Error fetching {session_id}: {e}\n")
    except Exception as e:
        if not quiet:
            sys.stderr.write(f"Error converting {session_id}: {e}\n")
    return False


def _dispatch_export_web(args, output_dir: Union[str, Path]):
    """Export web sessions from Claude.ai."""
    try:
        token, org_uuid = resolve_web_credentials()
        sessions = fetch_web_sessions(token, org_uuid)
        if not sessions:
        ...
        if not quiet:
            sys.stderr.write(f"Exporting {len(sessions)} web sessions...\n")

        for session in sessions:
            session_id = session.get("uuid", session.get("id"))
            if session_id:
                _export_single_web_session(
                    token, org_uuid, session_id, web_subdir, minimal, quiet, include_source
                )
...
def _dispatch_export(args):
    ...
    raw_targets = args.target if args.target else []
    targets = []
    for t in raw_targets:
        if t.endswith(".jsonl"):
            ...
            targets.append(_coerce_target_to_workspace_pattern(t))

    name_patterns = getattr(args, "name_patterns", None) or []
    if name_patterns:
        targets.extend(name_patterns)

    project_names = getattr(args, "projects", None) or []
    handled_project = False
    for project_name in project_names:
        cmd_project_export(project_name, output_dir, args)
        handled_project = True
    ...
    if not project_names:
        ...
        if alias_name:
            cmd_project_export(alias_name, output_dir, args)
            alias_handled = True

    if alias_handled or (
        handled_project and not targets and not getattr(args, "all_workspaces", False)
    ):
        return
    ...
    if len(targets) == 1 and isinstance(targets[0], str) and targets[0].endswith(".jsonl"):
...
def _dispatch_project(args):
    ...
    if project_cmd == "list" or project_cmd is None:
        cmd_project_list(args)
    elif project_cmd == "show":
        cmd_project_show(args)
    elif project_cmd == "create":
        cmd_project_create(args)
    elif project_cmd == "delete":
        cmd_project_delete(args)
    elif project_cmd == "add":
        cmd_project_add(args)
    elif project_cmd == "remove":
        cmd_project_remove(args)
    elif project_cmd == "export":
        _apply_home_flags_from_names(args, getattr(args, "homes", []) or [])
        cmd_project_export(args.name, args.output_dir, args)
    elif project_cmd == "stats":
        _apply_home_flags_from_names(args, getattr(args, "homes", []) or [])
        stats_args = SimpleNamespace(**vars(args))
        stats_args.workspace = [f"@{args.name}"]
        stats_args.all_workspaces = False
        _apply_project_sources_to_args(stats_args, args.name)
        _dispatch_stats(stats_args)
    elif project_cmd == "config-export":
        cmd_project_config_export(args)
    elif project_cmd == "config-import":
        cmd_project_config_import(args)
...
def _build_sync_args(args, remotes: list) -> SyncCommandArgs:
    ...
    return SyncCommandArgs(
        ...
        remotes=remotes,
        force=getattr(args, "force", False) or getattr(args, "sync", False),
        all_homes=getattr(args, "all_homes", False),
        jobs=getattr(args, "jobs", 1),
        no_remote=getattr(args, "no_remote", False),
        no_wsl=getattr(args, "no_wsl", False),
...
def _expand_patterns_for_sync(patterns: list[str]) -> list[str]:
    """Expand @project patterns to concrete workspaces for sync."""
    if not patterns:
        return [""]
    ...
    expanded: list[str] = []
    for pat in patterns:
        # Allow "=@project" (from exact matching) to behave like "@project"
        pattern = pat[1:] if pat.startswith("=@") else pat
        if pattern.startswith("@"):
            proj = aliases.get(pattern[1:])
            if proj:
                for workspaces in proj.values():
                    for ws in workspaces:
                        expanded.append(ws)
                        # Add decoded path using filesystem when available to preserve hyphens
                        decoded_fs = normalize_workspace_name(ws, verify_local=True)
                        if decoded_fs and decoded_fs not in expanded:
                            expanded.append(decoded_fs)
                        # Also add a best-effort decoded path without filesystem (fallback)
                        decoded = normalize_workspace_name(ws, verify_local=False)
                        if decoded and decoded not in expanded:
                            expanded.append(decoded)
                        # Add short workspace name for agents that store only the basename
                        base = get_workspace_name_from_path(ws)
...
def _build_stats_args(args) -> StatsCommandArgs:
    ...
    return StatsCommandArgs(
        ...
        models=getattr(args, "models", False),
...
def _dispatch_stats(args):
    ...
    # In isolated/test environments, skip cross-platform sources to avoid slow lookups.
    if os.environ.get("AGENT_HISTORY_HOME"):
        args.no_wsl = True
        args.no_windows = True
        args.no_remote = True
    ...
    args.workspace = combined

    # Guard: require project or explicit pattern for cross-home access
    has_windows = getattr(args, "windows", False)
    has_wsl = getattr(args, "wsl", False)
    has_remotes = bool(getattr(args, "remotes", None))
    all_homes = getattr(args, "all_homes", False)
    all_workspaces = getattr(args, "all_workspaces", False)
    needs_cross_home = bool(has_windows or has_wsl or has_remotes or all_homes)
    if needs_cross_home and not all_workspaces and not combined:
        if has_windows:
            target = "Windows"
        elif has_wsl:
            target = "WSL"
    ...
    if not no_sync:
        ...
        # Separator between sync progress and stats output
        print()
...
def _dispatch_command(args):
    ...
    if cmd == "home":
        ...
        verb = getattr(args, "session_verb", None) or "list"
        if verb == "show":
            _dispatch_session_show(args)
        elif verb == "export":
            _dispatch_export(args)
        elif verb == "stats":
            _dispatch_stats(args)
...
def _insert_default_verb(argv: list[str]) -> list[str]:
    """Insert default 'list' verb when omitted for noun commands."""
    if not argv:
        return argv

    verb_map = {
        "home": {"list", "show", "add", "remove", "export", "stats"},
        "ws": {"list", "show", "export", "stats"},
        "session": {"list", "show", "export", "stats"},
        "project": {
            "list",
            "show",
            "add",
    ...
    next_token = argv[1]
    if next_token.startswith("-") or next_token not in verbs:
        return [cmd, "list", *argv[1:]]
    return argv
...
def main():
    ...
    remaining = _insert_default_verb(remaining)
    ...
    # Set global table width from --width/-w option
    if hasattr(args, "width") and args.width is not None:
        set_table_width(args.width)
